...
import com.rainbowland.core.utils.ShortArray;
...
public final class EntityTransmuter {
	...
	private final Array<TransmuteOperation> operations;

	private final ShortArray entityToIdentity;
	...
	EntityTransmuter(World world, Aspect aspect) {
		this(world, new BitSet(aspect.allSet), new BitSet(aspect.exclusionSet));
	}
	...
	EntityTransmuter(World world, BitSet additions, BitSet removals) {
		...
		operations = new Array<TransmuteOperation>(TransmuteOperation.class);
	...
	static class Factory {
		...
		private Array<ComponentMapper> getAdditions(BitSet origin) {
			ComponentTypeFactory tf = cm.typeFactory;
			Array<ComponentMapper> types = new Array(ComponentMapper.class);
		...
		private Array<ComponentMapper> getRemovals(BitSet origin) {
			ComponentTypeFactory tf = cm.typeFactory;
			Array<ComponentMapper> types = new Array(ComponentMapper.class);
	...
	static class TransmuteOperation {
		...
		public TransmuteOperation(int compositionId,
		                          Array<ComponentMapper> additions,
		                          Array<ComponentMapper> removals) {
		...
		Array<Class<? extends Component>> getAdditions(Array<Class<? extends Component>> out) {
			for (int i = 0, s = additions.length; s > i; i++) {
