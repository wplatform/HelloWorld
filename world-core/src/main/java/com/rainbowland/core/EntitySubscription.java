...
import com.rainbowland.core.annotations.DelayedComponentRemoval;
...
public class EntitySubscription {
    ...
    private final IntArray entities;
    ...
    void process(IntArray changed, IntArray deleted) {
        deleted(deleted);
        changed(changed);
    ...
    void processAll(IntArray changed, IntArray deleted) {
        deletedAll(deleted);
        changed(changed);
    ...
    private void transferBitsToInts(IntArray inserted, IntArray removed) {
    ...
    private void changed(IntArray entitiesWithCompositions) {
    ...
    private void deletedAll(IntArray entities) {
        int[] ids = entities.getData();
        for (int i = 0, s = entities.size(); s > i; i++) {
            int id = ids[i];
    ...
    public interface SubscriptionListener {
         ...
         */
        void inserted(IntArray entities);
         ...
         */
        void removed(IntArray entities);
    }

    public static class SubscriptionExtra {
        final IntArray inserted = new IntArray();
        final IntArray removed = new IntArray();
        final Aspect aspect;
        final Aspect.Builder aspectReflection;
        final Array<SubscriptionListener> listeners = new Array<SubscriptionListener>();
