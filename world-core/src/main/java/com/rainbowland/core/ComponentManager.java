...
import com.rainbowland.core.utils.*;
...
public class ComponentManager extends BaseSystem {
	...
	private Array<ComponentMapper> mappers = new Array(ComponentMapper.class);

	private final ComponentIdentityResolver identityResolver = new ComponentIdentityResolver();
	final ShortArray entityToIdentity;
	...
	protected ComponentManager(int entityContainerSize) {
		entityToIdentity = new ShortArray(entityContainerSize);
	 ...
	 */
	void clean(IntArray pendingPurge) {
	...
	private void removeComponents(int entityId) {
		Array<ComponentMapper> mappers = componentMappers(entityId);
	 ...
	 */
	protected Array<Component> getComponentsByType(ComponentType type) {
   ...
   /**
	 ...
	 */
	public ImmunityArray<ComponentType> getComponentTypes() {
	 ...
	 */
	public Array<Component> getComponentsFor(int entityId, Array<Component> fillArray) {
		Array<ComponentMapper> mappers = componentMappers(entityId);

		for (int i = 0, s = mappers.size(); s > i; i++) {
			fillArray.add(mappers.get(i).get(entityId));
		}
	...
	/** Get component composition of entity. */
	private Array<ComponentMapper> componentMappers(int entityId) {
		int identityIndex = entityToIdentity.get(entityId);
	...
	void synchronize(EntitySubscription es) {
		Array<BitSet> compositionBits = identityResolver.compositionBits;
	...
	static final class ComponentIdentityResolver {
		final Array<BitSet> compositionBits;
		final Array<Array<ComponentMapper>> compositionMappers;

		ComponentIdentityResolver() {
			compositionBits = new Array(BitSet.class);
			compositionBits.add(new BitSet());
			compositionMappers = new Array<Array<ComponentMapper>>();
			compositionMappers.add(new Array(ComponentMapper.class));
		}
		...
		int allocateIdentity(BitSet componentBits, ComponentManager cm) {
			Array<ComponentMapper> mappers =
				new Array<ComponentMapper>(ComponentMapper.class, componentBits.cardinality());
			...
			compositionBits.add(new BitSet(componentBits));
