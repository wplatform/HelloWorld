...
import com.rainbowland.core.injection.Injector;
...
import com.rainbowland.core.utils.EntityBuilder;

import java.util.*;

import static com.rainbowland.core.WorldConfiguration.ASPECT_SUBSCRIPTION_MANAGER_IDX;
import static com.rainbowland.core.WorldConfiguration.COMPONENT_MANAGER_IDX;
import static com.rainbowland.core.WorldConfiguration.ENTITY_MANAGER_IDX;
...
public class World {
	...
	final Array<BaseSystem> systemsArray;
	...
	public World(WorldConfiguration configuration) {
		...
		systemsArray = configuration.systems;

		final ComponentManager lcm =
			(ComponentManager) systemsArray.get(COMPONENT_MANAGER_IDX);
		final EntityManager lem =
			(EntityManager) systemsArray.get(ENTITY_MANAGER_IDX);
		final AspectSubscriptionManager lasm =
			(AspectSubscriptionManager) systemsArray.get(ASPECT_SUBSCRIPTION_MANAGER_IDX);
	...
	public void dispose() {
		...
		for (BaseSystem system : systemsArray) {
			try {
				system.dispose();
			} catch (Exception e) {
				exceptions.add(e);
	 ...
	 */
	public ImmunityArray<BaseSystem> getSystems() {
		return systemsArray;
	}
	...
	protected void setInvocationStrategy(SystemInvocationStrategy invocationStrategy) {
		...
		invocationStrategy.setSystems(systemsArray);
	...
	public void process() {
		...
		IntArray pendingPurge = batchProcessor.getPendingPurge();
