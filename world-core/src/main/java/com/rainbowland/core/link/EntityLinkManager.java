...
public class EntityLinkManager extends BaseEntitySystem {
	...
	final Array<LinkSite> linkSites = new Array<LinkSite>();
	final Array<LinkSite> decoratedLinkSites = new Array<LinkSite>();
	...
	private void process(Array<LinkSite> sites) {
	...
	private static class LinkCreateListener implements ComponentTypeFactory.ComponentTypeListener {
		...
		@Override
		public void initialize(Array<ComponentType> types) {
		...
		public void onCreated(ComponentType type) {
			Array<LinkSite> links = linkFactory.create(type);
			if (links.isEmpty())
				return;
