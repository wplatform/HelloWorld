...
public class SpellScripts {
    ...
    @ReadingConverter
    public static class RowMapper implements Converter<Row, SpellScripts> {

        public SpellScripts convert(Row row) {
            SpellScripts domain = new SpellScripts();
            domain.setId(row.get("id", Integer.class));
            domain.setEffIndex(row.get("effIndex", Integer.class));
    ...
    @WritingConverter
    public static class ParamMapper implements Converter<SpellScripts, OutboundRow> {

        public OutboundRow convert(SpellScripts source) {
            OutboundRow row = new OutboundRow();
            Optional.ofNullable(source.getId()).ifPresent(e -> row.put("id", Parameter.from(e)));
            Optional.ofNullable(source.getEffIndex()).ifPresent(e -> row.put("effIndex", Parameter.from(e)));
