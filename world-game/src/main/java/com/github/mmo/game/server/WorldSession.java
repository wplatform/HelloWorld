...
import com.github.mmo.game.entity.item.ItemDynamicFieldGems;
import com.github.mmo.game.entity.item.ItemEnchantmentManager;
import com.github.mmo.game.entity.item.ItemPosCount;
import com.github.mmo.game.entity.player.model.TradeData;
import com.github.mmo.game.map.grid.Cell;
import com.github.mmo.game.movement.generator.FlightPathMovementGenerator;
import com.github.mmo.game.movement.model.MovementInfo;
import com.github.mmo.game.entity.object.WorldObject;
import com.github.mmo.game.entity.player.*;
import com.github.mmo.game.entity.taxi.TaxiPathGraph;
...
import com.github.mmo.game.battlepay.*;
...
import com.github.mmo.game.scripting.interfaces.iconversation.*;
import com.github.mmo.game.movement.*;
import com.github.mmo.game.networking.packet.mythicplus.*;
import com.github.mmo.game.ai.*;
import Framework.realm.*;
import com.github.mmo.game.networking.packet.quest.*;
import com.github.mmo.game.scripting.interfaces.iquest.*;
import com.github.mmo.game.scripting.interfaces.iitem.*;
import com.github.mmo.game.supportsystem.*;
import com.github.mmo.game.account.*;
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void handleBattlemasterJoin(BattlemasterJoin battlemasterJoin)
	{
		...
		if (global.getDisableMgr().isDisabledFor(DisableType.Battleground, bgQueueTypeId.battlemasterListId, null) || battlemasterListEntry.flags.HasAnyFlag(BattlemasterListFlags.disabled))
		{
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void handleCalendarInvite(CalendarInvitePkt calendarInvite)
	{
		...
		TeamFaction inviteeTeam = TeamFaction.forValue(0);
		...
		if (!result.isEmpty())
		{
			do
			{
				...
				if (!charResult.isDeletedCharacters)
				{
					...
					if (!charInfo.flags.HasAnyFlag(CharacterFlags.CharacterLockedForTransfer.getValue() | CharacterFlags.LockedByBilling.getValue()))
					{
										...
										}).WithChainingCallback((queryCallback, result) ->
										{
											...
											void finalizeCharacterCreation(SQLResult result1)
											{
												...
												if (result1 != null && !result1.isEmpty() && result.GetFieldCount() >= 3)
												{
													...
													{
														TeamFaction accTeam = TeamFaction.forValue(0);
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
						(var hasAppearance, _) = CollectionMgr.hasItemAppearance((int)saveEquipmentSet.set.Appearances[i]);
		...
		if (oldRace != factionChangeInfo.raceID)
		{
			...
			if (factionChangeInfo.factionChange)
			{
				...
				for (var it : global.getObjectMgr().factionChangeAchievements.entrySet())
				{
					...
					stmt.AddValue(0, (short)(newTeamId == TeamIds.Alliance ? achiev_alliance : achiev_horde));
					stmt.AddValue(1, lowGuid);
					trans.append(stmt);

					stmt = DB.characters.GetPreparedStatement(CharStatements.UPD_CHAR_ACHIEVEMENT);
					stmt.AddValue(0, (short)(newTeamId == TeamIds.Alliance ? achiev_alliance : achiev_horde));
					stmt.AddValue(1, (short)(newTeamId == TeamIds.Alliance ? achiev_horde : achiev_alliance));
					stmt.AddValue(2, lowGuid);
					trans.append(stmt);
				}

				// Item conversion
				var itemConversionMap = newTeamId == TeamIds.Alliance ? global.getObjectMgr().FactionChangeItemsHordeToAlliance : global.getObjectMgr().factionChangeItemsAllianceToHorde;
	...
	private void handleSocketGems(SocketGems socketGems)
	{
		...
		for (int i = 0; i < ItemConst.MaxGemSockets; ++i) //check for hack maybe
		{
			...
			if (ItemConst.SocketColorToGemTypeMask[itemTarget.getSocketColor(i).getValue()] != gemProperties[i].type)
			{
				...
				if (!ItemConst.SocketColorToGemTypeMask[itemTarget.getSocketColor(i).getValue()].HasAnyFlag(SocketColor.Prismatic) || !gemProperties[i].type.HasAnyFlag(SocketColor.Prismatic))
				{
					return;
				}
			}
		...
		for (int slot = 0; slot < rawItemCount; ++slot)
		{
			...
			if (vendorItem.getType() == ItemVendorType.item)
			{
				...
				if (!getPlayer().isGameMaster())
				{
					...
					if ((itemTemplate.hasFlag(ItemFlags2.FactionHorde) && getPlayer().getTeam() == TeamFaction.Alliance) || (itemTemplate.hasFlag(ItemFlags2.FactionAlliance) && getPlayer().getTeam() == TeamFaction.Horde))
					{
						continue;
					}
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	...
	{
		return !(_socket[ConnectionType.realm.getValue()] != null && _socket[ConnectionType.realm.getValue()].IsOpen() && _socket[ConnectionType.instance.getValue()] != null && _socket[ConnectionType.instance.getValue()].IsOpen());
	}
	...
	public WorldSession(int id, String name, int battlenetAccountId, WorldSocket sock, AccountTypes sec, Expansion expansion, long mute_time, String os, Locale locale, int recruiter, boolean isARecruiter)
	{
		...
		recvQueue = new(ProcessQueue, new ExecutionDataflowBlockOptions() {MaxDegreeOfParallelism = 10, EnsureOrdered = true, CancellationToken = cancellationToken.Token});
	...
	public final void logoutPlayer(boolean save)
	{
		...
		if (_socket[ConnectionType.instance.getValue()] != null)
		{
			_socket[ConnectionType.instance.getValue()].CloseSocket();
			_socket[ConnectionType.instance.getValue()] = null;
		}
	...
	public final boolean updateWorld(int diff)
	{
		...
		if ((_socket[ConnectionType.realm.getValue()] != null && !_socket[ConnectionType.realm.getValue()].IsOpen()) || (_socket[ConnectionType.instance.getValue()] != null && !_socket[ConnectionType.instance.getValue()].IsOpen()))
		{
	...
	{
		_socket[ConnectionType.instance.getValue()] = sock;
	}
	...
	public final void sendConnectToInstance(ConnectToSerial serial)
	{
		...
		instanceConnectKey.connectionType = ConnectionType.instance;
		...
		connectTo.con = (byte)ConnectionType.instance.getValue();
