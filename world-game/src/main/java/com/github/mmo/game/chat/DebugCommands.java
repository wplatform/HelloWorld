...
//			}
    ...
    private static boolean handleDebugWarModeBalanceCommand(CommandHandler handler, String command, Integer rewardValue) {
        ...
        switch (command) {
            case "alliance":
                global.getWorldMgr().setForcedWarModeFactionBalanceState(TeamIds.Alliance, (rewardValue == null ? 0 : rewardValue.intValue()));

                break;
            case "horde":
                global.getWorldMgr().setForcedWarModeFactionBalanceState(TeamIds.Horde, (rewardValue == null ? 0 : rewardValue.intValue()));
        ...
        private static boolean handleDebugWarModeFactionBalanceCommand(CommandHandler handler, String command) {
            ...
            switch (command.toLowerCase()) {
                default: // workaround for Variant of only ExactSequences not being supported
                    ...
                    return false;
                case "alliance":
                    global.getWorldMgr().setForcedWarModeFactionBalanceState(TeamIds.Alliance, rewardValue);

                    break;
                case "horde":
                    global.getWorldMgr().setForcedWarModeFactionBalanceState(TeamIds.Horde, rewardValue);
