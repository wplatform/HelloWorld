...
import com.github.mmo.game.chat.CustomChatTextBuilder;
import com.github.mmo.game.combat.CombatManager;
import com.github.mmo.game.combat.CombatReference;
...
import com.github.mmo.game.entity.unit.enums.*;
import com.github.mmo.game.entity.vehicle.Vehicle;
import com.github.mmo.game.group.PlayerGroup;
import com.github.mmo.game.loot.Loot;
import com.github.mmo.game.loot.LootManager;
import com.github.mmo.game.loot.LootStorage;
import com.github.mmo.game.map.*;
import com.github.mmo.game.map.grid.CellCoord;
import com.github.mmo.game.map.grid.MapDefines;
import com.github.mmo.game.map.grid.LiquidData;
...
import com.github.mmo.game.movement.model.MovementForces;
...
import com.github.mmo.game.scripting.interfaces.ispell.ISpellCalculateBonusCoefficient;
import com.github.mmo.game.scripting.interfaces.ispell.ISpellCalculateMultiplier;
import com.github.mmo.game.scripting.interfaces.iunit.*;
import com.github.mmo.game.spell.*;
import com.github.mmo.game.spell.auras.AuraApplicationCollection;
import com.github.mmo.game.spell.auras.AuraCollection;
import com.github.mmo.game.spell.auras.enums.AuraType;
import com.github.mmo.game.spell.events.DelayedCastEvent;
...
import static com.github.mmo.game.entity.object.update.ObjectFields.*;
import static com.github.mmo.game.entity.unit.UnitDefine.*;
...
public class Unit extends WorldObject
{
	...
	*/
	...
	damage = tempRef_damage2.refArgValue;
		...
		if (!damageInfo.outArgValue.getHitInfo().HasAnyFlag(hitInfo.Miss))
		{
	...
	private MeleeHitOutcome rollMeleeOutcomeAgainst(Unit victim, WeaponAttackType attType)
	{
		...
		if (attackerLevel >= victimLevel + 4 && !isControlledByPlayer() && !(getTypeId() == TypeId.UNIT && toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.NoCrushingBlows)))
		{
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	{
		return playerClass.forValue((byte)getUnitData().classId);
	}
	...
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.SUMMON);
	}

	public final boolean isGuardian()
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.Guardian);
	}

	public final boolean isPet()
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.pet);
	}

	public final boolean isHunterPet()
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.HunterPet);
	}

	public final boolean isTotem()
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.totem);
	}

	public final boolean isVehicle()
	{
		return getUnitTypeMask().HasAnyFlag(UnitTypeMask.vehicle);
	}
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	public unit(boolean isWorldObject)
	{
		...
		for (byte i = 0; i < UnitMods.End.getValue(); ++i)
		{
		...
		getAuraPctModifiersGroup()[UnitMods.DamageOffHand.getValue()][UnitModifierPctType.Total.getValue()] = 0.5f;
...
//			}
	...
	public final int getModelForForm(ShapeShiftForm form, int spellId)
	{
		...
		if (formEntry != null && formEntry.CreatureDisplayID[0] != 0)
		{
			...
			{
				if (player.teamForRace(getRace()) == TeamFaction.Alliance)
				{
				...
				if (modelid == 0 && player.teamForRace(getRace()) == TeamFaction.Horde)
				{
					modelid = formEntry.CreatureDisplayID[0];
				}
			}
	...
	public final boolean isInPartyWith(Unit unit)
	{
		...
		else if ((u2.isTypeId(TypeId.PLAYER) && u1.isTypeId(TypeId.UNIT) && u1.toCreature().getTemplate().typeFlags.HasAnyFlag(CreatureTypeFlags.TreatAsRaidUnit)) || (u1.isTypeId(TypeId.PLAYER) && u2.isTypeId(TypeId.UNIT) && u2.toCreature().getTemplate().typeFlags.HasAnyFlag(CreatureTypeFlags.TreatAsRaidUnit)))
		{
			return true;
		}
		...
		if (damagetype != DamageEffectType.NoDamage)
		{
			...
			if (damageTaken == 0 && damagetype != DamageEffectType.DOT && cleanDamage != null && cleanDamage.getAbsorbedDamage() != 0)
			{
				if (victim != attacker && victim.isPlayer())
				{
					...
					if (spell != null)
					{
						if (spell.getState() == SpellState.Preparing && spell.spellInfo.getInterruptFlags().HasAnyFlag(SpellInterruptFlags.DamageAbsorb))
						{
...
//								}
	...
	public static double calcArmorReducedDamage(Unit attacker, Unit victim, double damage, SpellInfo spellInfo, WeaponAttackType attackType, int attackerLevel)
	{
		...
		if (attacker != null)
		{
			...
			{
				if (eff.getMiscValue().HasAnyFlag(spellSchoolMask.NORMAL.getValue()) && eff.isAffectingSpell(spellInfo))
				{
	...
	public final double meleeDamageBonusTaken(Unit attacker, double pdamage, WeaponAttackType attType, DamageEffectType damagetype, SpellInfo spellProto, SpellSchoolMask damageSchoolMask)
	{
		...
		if (spellProto != null)
		{
			...
			if (spellProto.getSpellFamilyName() == SpellFamilyNames.Druid && spellProto.getSpellFamilyFlags().get(0).HasAnyFlag(0x00008800))
			{
...
//			};
		...
		{
												if (!aurEff.miscValue.HasAnyFlag(spellSchoolMask.NORMAL.getValue()))
												{
													return false;
												}
	...
	{
		return interruptMask.HasAnyFlag(flags);
	}

	private boolean hasInterruptFlag(SpellAuraInterruptFlags2 flags)
	{
		return interruptMask2.HasAnyFlag(flags);
	}
	...
	private void dealMeleeDamage(CalcDamageInfo damageInfo, boolean durabilityLoss)
	{
		...
		if (damageInfo.getTargetState() == victimState.Parry && (!victim.isCreature() || victim.toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.NoParryHasten)))
		{
			// Get attack timers
	...
	private double[][] auraFlatModifiersGroup = new double[UnitMods.End.getValue()][];
	...
	private double[][] auraPctModifiersGroup = new double[UnitMods.End.getValue()][];
	...
	{
		return getLiquidStatus().HasAnyFlag(ZLiquidStatus.InWater.getValue() | ZLiquidStatus.UnderWater.getValue());
	}
	...
	public final MountCapabilityRecord getMountCapability(int mountType)
	{
		...
		isInWater = liquidStatus.HasAnyFlag(ZLiquidStatus.InWater.getValue() | ZLiquidStatus.UnderWater.getValue());
	...
	private boolean initTamedPet(Pet pet, int level, int spell_id)
	{
		...
		if (isTypeId(TypeId.PLAYER))
		{
			pet.setUnitFlag(com.github.mmo.game.entity.unit.enums.UnitFlag.PlayerControlled);
		}
	...
	public final double spellDamageBonusTaken(Unit caster, SpellInfo spellProto, double pdamage, DamageEffectType damagetype)
	{
		...
		if (cheatDeath != null)
		{
			if (cheatDeath.miscValue.HasAnyFlag(spellSchoolMask.NORMAL.getValue()))
			{
				tangible.RefObject<Double> tempRef_TakenTotalMod = new tangible.RefObject<Double>(TakenTotalMod);
				MathUtil.AddPct(tempRef_TakenTotalMod, cheatDeath.amount);
			TakenTotalMod = tempRef_TakenTotalMod.refArgValue;
	...
	public final double spellHealingBonusTaken(Unit caster, SpellInfo spellProto, double healamount, DamageEffectType damagetype)
	{
		...
		// Nourish cast
		if (spellProto.getSpellFamilyName() == SpellFamilyNames.Druid && spellProto.getSpellFamilyFlags().get(1).HasAnyFlag(0x2000000))
		{
...
//			}
	...
	public static ProcFlagsHit createProcHitMask(SpellNonMeleeDamage damageInfo, SpellMissInfo missCondition)
	{
		...
		else
		{
			...
			var damageNullified = damageInfo.hitInfo.HasAnyFlag(hitInfo.FullAbsorb.getValue() | hitInfo.FullResist.getValue()) || hitMask.HasAnyFlag(ProcFlagsHit.fullBlock);

			if (!damageNullified)
			{
				// On crit
				if (damageInfo.hitInfo.HasAnyFlag(SpellHitType.crit.getValue()))
				{
			...
			else if (damageInfo.hitInfo.HasAnyFlag(hitInfo.FullResist.getValue()))
			{
				hitMask = ProcFlagsHit.forValue(hitMask.getValue() | ProcFlagsHit.FullResist.getValue());
			}
		}
...
//			}
	...
	public final void castStop(int except_spellid)
	{
		...
		{
			if ((getCurrentSpells().containsKey(i) && (var spell = getCurrentSpells().get(i)) == var spell) && spell != null && spell.spellInfo.id != except_spellid)
			{
...
//			}
	...
	public final boolean applyDiminishingToDuration(SpellInfo auraSpellInfo, tangible.RefObject<Integer> duration, WorldObject caster, DiminishingLevels previousLevel)
	{
		...
		{
			case Taunt:
				if (isTypeId(TypeId.UNIT) && toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.ObeysTauntDiminishingReturns))
				{
...
// C# TO JAVA CONVERTER TASK: The C# 'struct' constraint has no equivalent in Java:
	...
	{
		_ApplyAura(aurApp, new HashSet<Integer>() {effIndex});
	}
	...
	public final void _ApplyAura(AuraApplication aurApp, HashSet<Integer> effMask)
	{
		...
		if (aState != 0)
		{
			...
			if (aStateMask.HasAnyFlag((int)AuraStateType.PerCasterAuraStateMask.getValue()) && (getUnitData().auraState & aStateMask) != 0)
			{
				getValues().modifyValue(getUnitData()).modifyValue(getUnitData().auraState);
				forceUpdateFieldChange();
			}
	...
	public final double getTotalAuraModValue(UnitMods unitMod)
	{
		if (unitMod.getValue() >= UnitMods.End.getValue())
		{
	...
	private void procSkillsAndReactives(boolean isVictim, Unit procTarget, ProcFlagsInit typeMask, ProcFlagsHit hitMask, WeaponAttackType attType)
	{
		...
		if (typeMask.hasFlag(procFlags.MeleeBasedTriggerMask) && procTarget)
		{
			...
			if (hitMask.HasAnyFlag(ProcFlagsHit.critical.getValue() | ProcFlagsHit.Parry.getValue().getValue() | ProcFlagsHit.Dodge.getValue().getValue().getValue() | ProcFlagsHit.Block.getValue().getValue().getValue()))
			{
				// for victim
				if (isVictim)
				{
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
	...
	private SpellSchool getSpellSchoolByAuraGroup(UnitMods unitMod)
	{
	...
	public final void handleStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, double amount, boolean apply)
	{
		if (unitMod.getValue() >= UnitMods.End.getValue() || modifierType.getValue() >= UnitModifierFlatType.End.getValue())
		{
	...
	public final void applyStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, double pct)
	{
		if (unitMod.getValue() >= UnitMods.End.getValue() || modifierType.getValue() >= UnitModifierPctType.End.getValue())
		{
	...
	public final void setStatFlatModifier(UnitMods unitMod, UnitModifierFlatType modifierType, double val)
	{
		if (getAuraFlatModifiersGroup()[unitMod.getValue()][modifierType.getValue()] == val)
		{
			return;
		}

		getAuraFlatModifiersGroup()[unitMod.getValue()][modifierType.getValue()] = val;
		updateUnitMod(unitMod);
	}

	public final void setStatPctModifier(UnitMods unitMod, UnitModifierPctType modifierType, double val)
	{
	...
	public final double getFlatModifierValue(UnitMods unitMod, UnitModifierFlatType modifierType)
	{
		if (unitMod.getValue() >= UnitMods.End.getValue() || modifierType.getValue() >= UnitModifierFlatType.End.getValue())
		{
	...
	public final double getPctModifierValue(UnitMods unitMod, UnitModifierPctType modifierType)
	{
		if (unitMod.getValue() >= UnitMods.End.getValue() || modifierType.getValue() >= UnitModifierPctType.End.getValue())
		{
			Log.outError(LogFilter.unit, "attempt to access non-existing modifier value from UnitMods!");
	...
	public final void updateStatBuffMod(Stats stat)
	{
		...
		var unitMod = UnitMods.StatStart + stat.getValue();
	...
	public void updateResistances(SpellSchool school)
	{
		...
		{
			var unitMod = UnitMods.ResistanceStart + school.getValue();
			var value = MathUtil.CalculatePct(getFlatModifierValue(unitMod, UnitModifierFlatType.base), Math.max(getFlatModifierValue(unitMod, UnitModifierFlatType.BasePCTExcludeCreate), -100.0f));
			value *= getPctModifierValue(unitMod, UnitModifierPctType.base);
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	private void updateUnitMod(UnitMods unitMod)
	{
		...
		switch (unitMod)
		{
			...
			case Fury:
			case Pain:
				updateMaxPower(powerType.forValue(unitMod - UnitMods.PowerStart));
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	private double getUnitCriticalChanceDone(WeaponAttackType attackType)
	{
		...
		{
			if (!toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.NoCrit))
			{
				chance = 5.0f;
	...
	private double getUnitParryChance(WeaponAttackType attType, Unit victim)
	{
		...
		{
			if (!victim.isTotem() && !victim.toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.NoParry))
			{
				chance = 6.0f;
	...
	private double getUnitBlockChance(WeaponAttackType attType, Unit victim)
	{
		...
		{
			if (!victim.isTotem() && !(victim.toCreature().getTemplate().flagsExtra.HasAnyFlag(CreatureFlagsExtra.NoBlock)))
			{
				chance = 3.0f;
