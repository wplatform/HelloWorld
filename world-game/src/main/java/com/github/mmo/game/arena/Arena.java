...
public class Arena extends Battleground {
    ...
    public void addPlayer(Player player) {
        ...
        {
            if (player.getEffectiveTeam() == TeamFaction.Horde) {
    ...
    public void endBattleground(TeamFaction winner) {
        ...
        if (isRated()) {
            ...
            var winnerArenaTeam = global.getArenaTeamMgr().getArenaTeamById(getArenaTeamIdForTeam(winner == 0 ? TeamFaction.Alliance : winner));
            var loserArenaTeam = global.getArenaTeamMgr().getArenaTeamById(getArenaTeamIdForTeam(winner == 0 ? TeamFaction.Horde : getOtherTeam(winner)));
            ...
            if (winnerArenaTeam != null && loserArenaTeam != null && winnerArenaTeam != loserArenaTeam) {
                ...
                if (winner != 0) {
                    ...
                    Log.outDebug(LogFilter.Arena, "Arena match Type: {0} for Team1Id: {1} - Team2Id: {2} ended. WinnerTeamId: {3}. Winner rating: +{4}, Loser rating: {5}", getArenaType(), getArenaTeamIdByIndex(TeamIds.Alliance), getArenaTeamIdByIndex(TeamIds.Horde), winnerArenaTeam.getId(), winnerChange, loserChange);
                ...
                // Deduct 16 points from each teams arena-rating if there are no winners after 45+2 minutes
                    ...
                    _arenaTeamScores[PvPTeamId.Alliance.getValue()].assign(winnerTeamRating, (int) (winnerTeamRating + SharedConst.ArenaTimeLimitPointsLoss), winnerMatchmakerRating, getArenaMatchmakerRating(TeamFaction.Alliance));
                    _arenaTeamScores[PvPTeamId.Horde.getValue()].assign(loserTeamRating, (int) (loserTeamRating + SharedConst.ArenaTimeLimitPointsLoss), loserMatchmakerRating, getArenaMatchmakerRating(TeamFaction.Horde));
