...
//			}
		...
		switch (e.getActionType())
		{
			...
			case SimpleTalk:
			{
				for (var target : targets)
				{
					...
					else if (isPlayer(target) && me != null)
					{
						...
						global.getCreatureTextMgr().sendChat(me, (byte)e.action.simpleTalk.textGroupId, isPlayer(templastInvoker) ? templastInvoker : null, ChatMsg.Addon, language.Addon, CreatureTextRange.NORMAL, 0, SoundKitPlayType.NORMAL, TeamFaction.other, false, target.toPlayer());
					}
			...
			case Cast:
			{
				...
				for (var target : targets)
				{
					...
					if (!e.action.cast.castFlags.HasAnyFlag((int)SmartCastFlags.AuraNotPresent.getValue()) || !target.toUnit().hasAura(e.action.cast.spell))
					{
			...
			case CrossCast:
			{
				...
				for (var caster : casters)
				{
					...
					for (var target : targets)
					{
						...
						if (!(e.action.crossCast.castFlags.HasAnyFlag((int)SmartCastFlags.AuraNotPresent.getValue())) || !target.toUnit().hasAura(e.action.crossCast.spell))
						{
							...
							casterUnit.castSpell(target.toUnit(), e.action.crossCast.spell, e.action.crossCast.castFlags.HasAnyFlag((int)SmartCastFlags.Triggered.getValue()));
						}
						else
						{
	...
	private boolean isSmart(Creature creature, boolean silent)
	{
		...
		if (!smart && !silent)
		{
			Log.outError(LogFilter.Sql, "SmartScript: Action target CREATURE (GUID: {0} Entry: {1}) is not using SmartAI, action skipped to prevent crash.", creature != null ? creature.getSpawnId() : (me != null ? me.getSpawnId() : 0), creature != null ? creature.getEntry() : (me != null ? me.getEntry() : 0));
		}
