	...
	public final void loadFromDB()
	{
		...
		// load template data from world DB
			...
			do
			{
				if (result.IsNull(2))
				{
					Log.outError(LogFilter.Sql, String.format("Table `quest_pool_members` contains reference to non-existing pool %1$s. Skipped.", result.<Integer>Read(1)));
		...
		for (var pair : lookup.entrySet())
		{
			...
			{
				Log.outError(LogFilter.Sql, String.format("Table `quest_pool_template` contains quest pool %1$s requesting %2$s spawns, but only has %3$s members. Requested spawns reduced.", pool.poolId, pool.numActive, pool.members.count));
			...
			if (!doRegenerate)
			{
				...
				for (var i = (int)pool.members.count; (i--) != 0;)
				{
					...
					if (member.isEmpty())
					{
						Log.outError(LogFilter.Sql, String.format("Table `quest_pool_members` contains no entries at index %1$s for quest pool %2$s. Index removed.", i, pool.poolId));
				...
				{
					doRegenerate = true;
					Log.outError(LogFilter.Sql, String.format("Table `pool_quest_save` has %1$s active members saved for pool %2$s, which requests %3$s active members. Pool spawns re-generated.", activeCount, pool.poolId, pool.numActive));
				}
			}
			...
			for (var memberKey : pool.members.keySet())
			{
				for (var quest : pool.Members[memberKey])
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `quest_pool_members` lists quest %1$s as member of pool %2$s, but it is already a member of pool %3$s. Skipped.", quest, pool.poolId, poolLookup.get(quest).poolId));
