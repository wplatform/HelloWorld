...
import com.github.mmo.game.networking.packet.InitWorldStates;
...
class HellfirePeninsulaPvP extends OutdoorPvP
{
	...
	{
		// add buffs
		if (player.getTeam() == TeamFaction.Alliance)
		{
	...
	public void handleKillImpl(Player killer, Unit killed)
	{
		...
		if (killer.getTeam() == TeamFaction.Alliance && killed.toPlayer().getTeam() != TeamFaction.Alliance)
		{
			killer.castSpell(killer, OutdoorPvPHPSpells.alliancePlayerKillReward, true);
		}
		else if (killer.getTeam() == TeamFaction.Horde && killed.toPlayer().getTeam() != TeamFaction.Horde)
		{
