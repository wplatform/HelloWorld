...
import game.*;
import com.github.mmo.game.entity.activePlayerData;
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
	...
	{
		if ((tree.entry.flags.HasAnyFlag(CriteriaTreeFlags.HordeOnly) && referencePlayer.getTeam() != TeamFaction.Horde) || (tree.entry.flags.HasAnyFlag(CriteriaTreeFlags.AllianceOnly) && referencePlayer.getTeam() != TeamFaction.Alliance))
		{
	...
	private boolean modifierSatisfied(ModifierTreeRecord modifier, long miscValue1, long miscValue2, WorldObject refe, Player referencePlayer)
	{
		...
		switch (ModifierTreeType.forValue(modifier.type))
		{
			...
			case PlayerFaction: // 116
			{
				...
				if (faction.factionGroup.HasAnyFlag((byte)FactionMasks.Horde.getValue()))
				{
					factionIndex = 0;
				}
				else if (faction.factionGroup.HasAnyFlag((byte)FactionMasks.Alliance.getValue()))
				{
					factionIndex = 1;
				}
				else if (faction.factionGroup.HasAnyFlag((byte)FactionMasks.player.getValue()))
				{
					factionIndex = 0;
				}
