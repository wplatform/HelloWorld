...
public class BattlegroundManager 
{
	...
	public final void loadBattlegroundTemplates()
	{
		...
		do
		{
			...
			if (bgTemplate.id != BattlegroundTypeId.AA && bgTemplate.id != BattlegroundTypeId.RB && bgTemplate.id != BattlegroundTypeId.RandomEpic)
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `battleground_template` for id %1$s contains a non-existing WorldSafeLocs.dbc id %2$s in field `AllianceStartLoc`. Ignoring.", bgTemplate.id, startId));
				}
				else
				{
					Log.outError(LogFilter.Sql, String.format("Table `Battleground_template` for Id %1$s has a non-existed WorldSafeLocs.dbc id %2$s in field `AllianceStartLoc`. BG not created.", bgTemplate.id, startId));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `battleground_template` for id %1$s contains a non-existing WorldSafeLocs.dbc id %2$s in field `HordeStartLoc`. Ignoring.", bgTemplate.id, startId));
				}
				else
				{
					Log.outError(LogFilter.Sql, String.format("Table `Battleground_template` for Id %1$s has a non-existed WorldSafeLocs.dbc id %2$s in field `HordeStartLoc`. BG not created.", bgTemplate.id, startId));
	...
	public final void loadBattleMastersEntry()
	{
		...
		do
		{
			...
			if (cInfo != null)
			{
				if (!cInfo.npcflag.HasAnyFlag((int)NPCFlags.BattleMaster.getValue()))
				{
	...
	private void checkBattleMasters()
	{
		...
		{
			if (creature.getValue().npcflag.HasAnyFlag((int)NPCFlags.BattleMaster.getValue()) && !mBattleMastersMap.containsKey(creature.getValue().entry))
			{
				Log.outError(LogFilter.Sql, "CreatureTemplate (Entry: {0}) has UNIT_NPC_FLAG_BATTLEMASTER but no data in `battlemaster_entry` table. Removing flag!", creature.getValue().entry);
