...
import com.github.mmo.game.entity.player.CollectionMgr;
...
import com.github.mmo.game.scripting.interfaces.iserver.*;
import com.github.mmo.game.scripting.interfaces.iworld.*;
...
VMap data directory is: {0}\\vmaps""", getDataPath());
	...
	{
		global.getWorldStateMgr().setValueAndSaveInDb(WorldStates.WarModeHordeBuffValue, 10 + (team == TeamIds.Alliance ? reward : 0), false, null);
		global.getWorldStateMgr().setValueAndSaveInDb(WorldStates.WarModeAllianceBuffValue, 10 + (team == TeamIds.Horde ? reward : 0), false, null);
	}
	...
	public final void sendGlobalMessage(ServerPacket packet, WorldSession self, TeamFaction team)
	{
	...
	public final void sendGlobalGMMessage(ServerPacket packet, WorldSession self, TeamFaction team)
	{
		for (var session : sessions.values())
		{
	...
	*/
	...
	public final void shutdownMsg(boolean show, Player player, String reason)
	{
		...
		if (shutdownMask.HasAnyFlag(ShutdownMask.IDLE))
		{
			return;
		}
		...
		if (show || (shutdownTimer < 5 * time.Minute && (_shutdownTimer % 15) == 0) || (shutdownTimer < 15 * time.Minute && (_shutdownTimer % time.Minute) == 0) || (shutdownTimer < 30 * time.Minute && (_shutdownTimer % (5 * time.Minute)) == 0) || (shutdownTimer < 12 * time.Hour && (_shutdownTimer % time.Hour) == 0) || (shutdownTimer > 12 * time.Hour && (_shutdownTimer % (12 * time.Hour)) == 0)) // > 12 h ; every 12 h
		{
			...
			var msgid = shutdownMask.HasAnyFlag(ShutdownMask.RESTART) ? ServerMessageType.RestartTime : ServerMessageType.ShutdownTime;

			sendServerMessage(msgid, str, player);
			Log.outDebug(LogFilter.Server, "Server is {0} in {1}", (shutdownMask.HasAnyFlag(ShutdownMask.RESTART) ? "restart" : "shuttingdown"), str);
		}
	}
	...
	public final int shutdownCancel()
	{
		...
		var msgid = shutdownMask.HasAnyFlag(ShutdownMask.RESTART) ? ServerMessageType.RestartCancelled : ServerMessageType.ShutdownCancelled;
		...
		Log.outDebug(LogFilter.Server, "Server {0} cancelled.", (shutdownMask.HasAnyFlag(ShutdownMask.RESTART) ? "restart" : "shutdown"));
...
// C# TO JAVA CONVERTER TASK: There is no Java ConcurrentHashMap equivalent to this .NET ConcurrentDictionary method:
	...
	private void updateGameTime()
	{
		...
		if (!isStopped && shutdownTimer > 0 && elapsed > 0)
		{
			...
			{
				if (!shutdownMask.HasAnyFlag(ShutdownMask.IDLE) || getActiveAndQueuedSessionCount() == 0)
				{
	...
	private void updateWarModeRewardValues()
	{
			...
			do
			{
				...
				if (raceEntry != null)
				{
					...
					if (raceFaction != null)
					{
						...
						{
							warModeEnabledFaction[TeamIds.Alliance] += result.<Long>Read(1);
						}
						else if ((raceFaction.factionGroup & (byte)FactionMasks.Horde.getValue()) != 0)
						{
							warModeEnabledFaction[TeamIds.Horde] += result.<Long>Read(1);
						}
					}
				}
			} while (result.NextRow());
		...
		global.getWorldStateMgr().setValueAndSaveInDb(WorldStates.WarModeHordeBuffValue, 10 + (dominantFaction == TeamIds.Alliance ? outnumberedFactionReward : 0), false, null);
		global.getWorldStateMgr().setValueAndSaveInDb(WorldStates.WarModeAllianceBuffValue, 10 + (dominantFaction == TeamIds.Horde ? outnumberedFactionReward : 0), false, null);
	}
}
