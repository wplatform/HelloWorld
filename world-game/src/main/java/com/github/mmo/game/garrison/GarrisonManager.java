...
public class GarrisonManager 
{
	...
	public final ArrayList<GarrAbilityRecord> rollFollowerAbilities(int garrFollowerId, GarrFollowerRecord follower, int quality, int faction, boolean initial)
	{
		...
		if (slots[1] > forcedTraits.size() + traitList.size())
		{
			...
			{
				if (genericTraits.get(i).flags.HasAnyFlag(GarrisonAbilityFlags.Exclusive))
				{
					break;
				}
			}
	...
	private void loadPlotFinalizeGOInfo()
	{
		...
		do
		{
			...
			if (goTemplate == null)
			{
				Log.outError(LogFilter.Sql, "Non-existing gameobject_template entry {0} was referenced in `garrison_plot_finalize_info`.`hordeGameObjectId` for garrPlotInstanceId {1}.", hordeGameObjectId, garrPlotInstanceId);
			...
			{
				Log.outError(LogFilter.Sql, "Invalid gameobject type {0} (entry {1}) was referenced in `garrison_plot_finalize_info`.`hordeGameObjectId` for garrPlotInstanceId {2}.", goTemplate.type, hordeGameObjectId, garrPlotInstanceId);
			...
			if (goTemplate == null)
			{
				Log.outError(LogFilter.Sql, "Non-existing gameobject_template entry {0} was referenced in `garrison_plot_finalize_info`.`allianceGameObjectId` for garrPlotInstanceId {1}.", allianceGameObjectId, garrPlotInstanceId);
			...
			if (goTemplate.type != GameObjectTypes.goober)
			{
				Log.outError(LogFilter.Sql, "Invalid gameobject type {0} (entry {1}) was referenced in `garrison_plot_finalize_info`.`allianceGameObjectId` for garrPlotInstanceId {2}.", goTemplate.type, allianceGameObjectId, garrPlotInstanceId);
			...
			{
				Log.outError(LogFilter.Sql, "Non-existing animKit.dbc entry {0} was referenced in `garrison_plot_finalize_info`.`hordeAnimKitId` for garrPlotInstanceId {1}.", hordeAnimKitId, garrPlotInstanceId);
			...
			{
				Log.outError(LogFilter.Sql, "Non-existing animKit.dbc entry {0} was referenced in `garrison_plot_finalize_info`.`allianceAnimKitId` for garrPlotInstanceId {1}.", allianceAnimKitId, garrPlotInstanceId);
