...
public class PathGenerator
{
	...
	private void buildPointPath(float[] startPoint, float[] endPoint)
	{
		...
		if (forceDestination && (!pathType.HasAnyFlag(PathType.NORMAL) || !inRange(getEndPosition(), getActualEndPosition(), 1.0f, 1.0f)))
		{
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# range operator:
			if ((steerPathFlags[ns].HasAnyFlag((byte)Detour.dtStraightPathFlags.DT_STRAIGHTPATH_OFFMESH_CONNECTION.getValue()) || !inRangeYZX(span[(new integer(ns * 3))..].ToArray(), startPos, minTargetDist, 1000.0f)))
			{
				break;
			}
