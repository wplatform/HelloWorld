...
public class BattlegroundQueue {
    ...
    public final GroupQueueInfo addGroup(Player leader, PlayerGroup group, TeamFaction team, PvpDifficultyRecord bracketEntry, boolean isPremade, int ArenaRating, int MatchmakerRating) {
        return addGroup(leader, group, team, bracketEntry, isPremade, ArenaRating, MatchmakerRating, 0);
    }

    public final GroupQueueInfo addGroup(Player leader, PlayerGroup group, TeamFaction team, PvpDifficultyRecord bracketEntry, boolean isPremade, int ArenaRating, int MatchmakerRating, int arenateamid) {
    ...
    public final int getAverageQueueWaitTime(GroupQueueInfo ginfo, BattlegroundBracketId bracket_id) {
        int team_index = TeamIds.Alliance; //default set to TeamIndex.Alliance - or non rated arenas!
    ...
    public final void battlegroundQueueUpdate(int diff, BattlegroundBracketId bracket_id, int arenaRating) {
        ...
        for (var bg : bgQueues) {
            ...
            if (!bg.isRated() && bg.getBracketId() == bracket_id && bg.getStatus() > BattlegroundStatus.WaitQueue.getValue() && bg.getStatus() < BattlegroundStatus.WaitLeave.getValue()) {
                ...
                for (var queueInfo : m_SelectionPools[TeamIds.Alliance].selectedGroups) {
                    inviteGroupToBG(queueInfo, bg, queueInfo.team);
                }

                for (var queueInfo : m_SelectionPools[TeamIds.Horde].selectedGroups) {
                    inviteGroupToBG(queueInfo, bg, queueInfo.team);
                }
        ...
        if (bg_template.isBattleground()) {
            if (checkPremadeMatch(bracket_id, MinPlayersPerTeam, MaxPlayersPerTeam)) {
                ...
                for (int i = 0; i < SharedConst.PvpTeamsCount; i++) {
                    for (var queueInfo : m_SelectionPools[TeamIds.Alliance + i].selectedGroups) {
        ...
        if (!m_queueId.rated) {
            ...
            if (found == 2) {
                ...
                if (aTeam.team != TeamFaction.Alliance) {
                    ...
                    m_QueuedGroups[bracket_id.getValue()][BattlegroundConst.BgQueuePremadeHorde].remove(queueArray[TeamIds.Alliance]);
                }

                if (hTeam.team != TeamFaction.Horde) {
                    m_QueuedGroups[bracket_id.getValue()][BattlegroundConst.BgQueuePremadeHorde].add(0, hTeam);
                    m_QueuedGroups[bracket_id.getValue()][BattlegroundConst.BgQueuePremadeAlliance].remove(queueArray[TeamIds.Horde]);
                }
    ...
    private boolean inviteGroupToBG(GroupQueueInfo ginfo, Battleground bg, TeamFaction side) {
        // set side if needed
        if (side != 0) {
            ginfo.team = side;
        }
    ...
    */
		...
		*/
        ...
        while (Math.abs(diffAli - diffHorde) > 1 && (m_SelectionPools[TeamIds.Horde].getPlayerCount() > 0 || m_SelectionPools[TeamIds.Alliance].getPlayerCount() > 0)) {
            ...
            if (diffAli < diffHorde) {
                ...
                if (m_SelectionPools[TeamIds.Alliance].kickGroup((int) (diffHorde - diffAli))) {
                    for (; alyIndex < aliCount && m_SelectionPools[TeamIds.Alliance].addGroup(m_QueuedGroups[bracket_id.getValue()][BattlegroundConst.BgQueueNormalAlliance].get(alyIndex), (int) ((aliFree >= diffHorde) ? aliFree - diffHorde : 0)); alyIndex++) {
                        ++alyIndex;
                    }
                }
                ...
                if (m_SelectionPools[TeamIds.Alliance].getPlayerCount() == 0) {
                    ...
                    m_SelectionPools[TeamIds.Horde].kickGroup((int) (diffHorde - diffAli));
                }
            } else {
                //kick horde group, add to pool new group if needed
                if (m_SelectionPools[TeamIds.Horde].kickGroup((int) (diffAli - diffHorde))) {
                    for (; hordeIndex < hordeCount && m_SelectionPools[TeamIds.Horde].addGroup(m_QueuedGroups[bracket_id.getValue()][BattlegroundConst.BgQueueNormalHorde].get(hordeIndex), (int) ((hordeFree >= diffAli) ? hordeFree - diffAli : 0)); hordeIndex++) {
                        ++hordeIndex;
                    }
                }
                ...
                if (m_SelectionPools[TeamIds.Horde].getPlayerCount() == 0) {
                    ...
                    m_SelectionPools[TeamIds.Alliance].kickGroup((int) (diffAli - diffHorde));
                }
            }
    ...
    private boolean checkNormalMatch(Battleground bg_template, BattlegroundBracketId bracket_id, int minPlayers, int maxPlayers) {
        ...
        int j = TeamIds.Alliance;

        if (m_SelectionPools[TeamIds.Horde].getPlayerCount() < m_SelectionPools[TeamIds.Alliance].getPlayerCount()) {
            j = TeamIds.Horde;
        }

        if (WorldConfig.getIntValue(WorldCfg.BattlegroundInvitationType) != BattlegroundQueueInvitationType.NoBalance.getValue() && m_SelectionPools[TeamIds.Horde].getPlayerCount() >= minPlayers && m_SelectionPools[TeamIds.Alliance].getPlayerCount() >= minPlayers) {
        ...
        if (global.getBattlegroundMgr().isTesting() && (m_SelectionPools[TeamIds.Alliance].getPlayerCount() != 0 || m_SelectionPools[TeamIds.Horde].getPlayerCount() != 0)) {
            return true;
        }
    ...
    private boolean checkSkirmishForSameFaction(BattlegroundBracketId bracket_id, int minPlayersPerTeam) {
        ...
        int teamIndex = TeamIds.Alliance;
        int otherTeam = TeamIds.Horde;
