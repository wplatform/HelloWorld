...
public class FormationMgr
{
	...
	public static void loadCreatureFormations()
	{
		...
		do
		{
			...
			// check data correctness
				if (global.getObjectMgr().getCreatureData(member.getLeaderSpawnId()) == null)
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("creature_formations table leader guid %1$s incorrect (not exist)", member.getLeaderSpawnId()));
					}
				...
				if (global.getObjectMgr().getCreatureData(memberSpawnId) == null)
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("creature_formations table member guid %1$s incorrect (not exist)", memberSpawnId));
					}
		...
		for (var leaderSpawnId : leaderSpawnIds)
		{
			...
			{
				Log.outError(LogFilter.Sql, String.format("creature_formation contains leader spawn %1$s which is not included on its formation, removing", leaderSpawnId));
