...
import com.github.mmo.game.loot.LootTemplate;
...
public final class ConditionManager {
    ...
    int objectType, int objectId),condition>objectVisibilityConditionStorage =new MultiMap<(
    int objectType, int objectId),condition>();
    ...
    public static boolean isPlayerMeetingCondition(Player player, PlayerConditionRecord condition) {
        ...
        if (levels != null) {
            var minLevel = (byte) (condition.flags.hasFlag(0x800) ? levels.getValue().MinLevelWithDelta : levels.getValue().minLevel);
            byte maxLevel = 0;

            if (!condition.flags.hasFlag(0x20)) {
                maxLevel = (byte) (condition.flags.hasFlag(0x800) ? levels.getValue().MaxLevelWithDelta : levels.getValue().maxLevel);
            }

            if (condition.flags.hasFlag(0x80)) {
