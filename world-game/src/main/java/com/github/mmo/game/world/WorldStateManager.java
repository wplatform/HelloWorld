...
import com.github.mmo.game.scripting.interfaces.iworldstate.*;
...
public class WorldStateManager 
{
	...
	public void loadFromDB()
	{
		...
		do
		{
			...
			if (!mapIds.isEmpty())
			{
				for (String mapIdToken : new LocalizedString(mapIds, ','))
				{
					...
					mapId = tempOut_mapId.outArgValue;
						Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with non-integer mapID (%2$s), map ignored", id, mapIdToken));
				...
				else
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with invalid mapID (%2$s), map ignored", id, mapId));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with nonempty MapIDs (%2$s) but no valid map id was found, ignored", id, mapIds));
			...
			if (!areaIds.isEmpty() && !worldState.mapIds.isEmpty())
			{
				for (String areaIdToken : new LocalizedString(areaIds, ','))
				{
					...
					areaId = tempOut_areaId.outArgValue;
						Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with non-integer areaID (%2$s), area ignored", id, areaIdToken));
				...
				else
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with invalid areaID (%2$s), area ignored", id, areaId));
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with areaID (%2$s) not on any of required maps, area ignored", id, areaId));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with nonempty AreaIDs (%2$s) but no valid area id was found, ignored", id, areaIds));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Table `world_state` contains a world state %1$s with nonempty AreaIDs (%2$s) but is a realm wide world state, area requirement ignored", id, areaIds));
			}
		...
		if (!result.isEmpty())
		{
			do
			{
				...
				if (worldState == null)
				{
					Log.outError(LogFilter.Sql, String.format("Table `world_state_value` contains a value for unknown world state %1$s, ignored", worldStateId));
