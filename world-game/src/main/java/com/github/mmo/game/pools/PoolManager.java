...

		{
			...
			else
			{
				...
				do
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("`pool_creature` has creature spawns on multiple different maps for creature guid (%1$s) in pool id (%2$s), skipped.", guid, pool_id));
...

		{
			...
			else
			{
				...
				do
				{
					...
					if (pPoolTemplate.mapId != data.getMapId())
					{
						Log.outError(LogFilter.Sql, String.format("`pool_gameobject` has gameobject spawns on multiple different maps for gameobject guid (%1$s) in pool id (%2$s), skipped.", guid, pool_id));
				...
				for (var(id, poolData) : poolTemplate)
				{
					...
					while (poolItr != 0)
					{
						if (poolData.mapId != -1)
						{
							...
							{
								Log.outError(LogFilter.Sql, String.format("`pool_pool` has child pools on multiple maps in pool id (%1$s), skipped.", poolItr));
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
		for (var(poolId, templateData) : poolTemplate)
		{
			if (isEmpty(poolId))
			{
				Log.outError(LogFilter.Sql, String.format("Pool Id %1$s is empty (has no creatures and no gameobects and either no child pools or child pools are all empty. The pool will not be spawned", poolId));
				...
				do
				{
					...
					if (!checkPool(pool_entry))
					{
						if (pool_pool_id != 0)
						{
							...
							Log.outError(LogFilter.Sql, "Pool Id {0} has no equal chance pooled entites defined and explicit chance sum is not 100. This broken pool is a child pool of Id {1} and cannot be safely removed.", pool_entry, result.<Integer>Read(2));
						}
						else
						{
