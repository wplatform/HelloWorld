...
import com.github.mmo.game.battlepet.*;

import com.github.mmo.game.entity.creature.CreatureTemplate;
import com.github.mmo.game.entity.player.Player;
import com.github.mmo.game.extendability.*;
import com.github.mmo.game.movement.*;
import com.github.mmo.game.scripting.interfaces.ispellmanager.*;
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
	...
	public static boolean canSpellTriggerProcOnEvent(SpellProcEntry procEntry, ProcEventInfo eventInfo)
	{
		...
		if (procEntry.getAttributesMask().HasAnyFlag(ProcAttributes.ReqPowerCost))
		{
	...
	public void loadSpellLearnSpells()
	{
				...
				for (var spellEffectInfo : entry.effects)
				{
					if (spellEffectInfo.effect == SpellEffectName.LearnSpell)
					{
						...
						for (var bound : db_node_bounds)
						{
							...
							{
								Log.outError(LogFilter.Sql, "Spell {0} auto-learn spell {1} in spell.dbc then the record in `spell_learn_spell` is redundant, please fix DB.", entry.id, dbc_node.spell);
		...
		for (var spellLearnSpell : CliDB.SpellLearnSpellStorage.values())
		{
			...
			for (var spellNode : db_node_bounds)
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Found redundant record (entry: %1$s, SpellID: %2$s) in `spell_learn_spell`, spell added automatically from SpellLearnSpell.db2", spellLearnSpell.spellID, spellLearnSpell.LearnSpellID));
					found = true;
	...
	public void loadSpellGroupStackRules()
	{
		...
		spellIds = tempOut_spellIds.outArgValue;
			...
			for (int spellId : spellIds)
			{
				...
				if (!found)
				{
					Log.outError(LogFilter.Sql, String.format("SpellId %1$s listed in `spell_group` with stack rule 3 does not share aura assigned for group %2$s", spellId, group_id));
				}
			}
		...
		if (!result.isEmpty())
		{
			do
			{
				...
				while (spellInfo != null)
				{
					...
					{
						Log.outError(LogFilter.Sql, "Spell {0} listed in `spell_proc` has duplicate entry in the table", spellInfo.getId());
					...
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has wrong `SchoolMask` set: {1}", spellInfo.getId(), procEntry.getSchoolMask());
					}

					if (procEntry.getSpellFamilyName() != 0 && !global.getDB2Mgr().IsValidSpellFamiliyName(procEntry.getSpellFamilyName()))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has wrong `SpellFamilyName` set: {1}", spellInfo.getId(), procEntry.getSpellFamilyName());
					}

					if (procEntry.getChance() < 0)
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has negative value in `Chance` field", spellInfo.getId());
						procEntry.setChance(0);
					}

					if (procEntry.getProcsPerMinute() < 0)
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has negative value in `ProcsPerMinute` field", spellInfo.getId());
						procEntry.setProcsPerMinute(0);
					}

					if (!procEntry.getProcFlags())
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} doesn't have `ProcFlags` value defined, proc will not be triggered", spellInfo.getId());
					}

					if ((boolean)(procEntry.getSpellTypeMask().getValue() & ~ProcFlagsSpellType.MaskAll.getValue()))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has wrong `SpellTypeMask` set: {1}", spellInfo.getId(), procEntry.getSpellTypeMask());
					}

					if (procEntry.getSpellTypeMask() != 0 && !procEntry.getProcFlags().hasFlag(procFlags.SpellMask))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has `SpellTypeMask` value defined, but it won't be used for defined `ProcFlags` value", spellInfo.getId());
					}

					if (procEntry.getSpellPhaseMask() == 0 && procEntry.getProcFlags().hasFlag(procFlags.ReqSpellPhaseMask))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} doesn't have `SpellPhaseMask` value defined, but it's required for defined `ProcFlags` value, proc will not be triggered", spellInfo.getId());
					}

					if ((boolean)(procEntry.getSpellPhaseMask().getValue() & ~ProcFlagsSpellPhase.MaskAll.getValue()))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has wrong `SpellPhaseMask` set: {1}", spellInfo.getId(), procEntry.getSpellPhaseMask());
					}

					if (procEntry.getSpellPhaseMask() != 0 && !procEntry.getProcFlags().hasFlag(procFlags.ReqSpellPhaseMask))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has `SpellPhaseMask` value defined, but it won't be used for defined `ProcFlags` value", spellInfo.getId());
					}

					if ((boolean)(procEntry.getHitMask().getValue() & ~ProcFlagsHit.MaskAll.getValue()))
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has wrong `HitMask` set: {1}", spellInfo.getId(), procEntry.getHitMask());
					}
					...
					{
						Log.outError(LogFilter.Sql, "`spell_proc` table entry for spellId {0} has `HitMask` value defined, but it won't be used for defined `ProcFlags` and `SpellPhaseMask` values", spellInfo.getId());
					}
					...
					for (var spellEffectInfo : spellInfo.getEffects())
					{
						...
						{
							Log.outError(LogFilter.Sql, String.format("The `spell_proc` table entry for spellId %1$s has DisableEffectsMask with effect %2$s, but effect %3$s is not an aura effect", spellInfo.getId(), spellEffectInfo.effectIndex, spellEffectInfo.effectIndex));
						}
					}
					...
					if (procEntry.getAttributesMask().hasFlag(ProcAttributes.ReqSpellmod))
					{
						...
						if (!found)
						{
							Log.outError(LogFilter.Sql, String.format("The `spell_proc` table entry for spellId %1$s has Attribute PROC_ATTR_REQ_SPELLMOD, but spell has no spell mods. Proc will not be triggered", spellInfo.getId()));
						}
					}

					if ((procEntry.getAttributesMask().getValue() & ~ProcAttributes.AllAllowed.getValue()) != 0)
					{
						Log.outError(LogFilter.Sql, String.format("The `spell_proc` table entry for spellId %1$s has `AttributesMask` value specifying invalid attributes 0x%2$X.", spellInfo.getId(), (procEntry.getAttributesMask().getValue() & ~ProcAttributes.AllAllowed.getValue())));
			...
			for (var spellInfo : kvp.VALUES)
			{
				...
				if (procSpellTypeMask == 0)
				{
					for (var spellEffectInfo : spellInfo.effects)
					{
						if (spellEffectInfo.isAura())
						{
							Log.outDebug(LogFilter.Sql, String.format("Spell Id %1$s has DBC ProcFlags 0x%2$X 0x%3$X, but it's of non-proc aura type, it probably needs an entry in `spell_proc` table to be handled correctly.", spellInfo.id, spellInfo.ProcFlags[0], spellInfo.ProcFlags[1]));
		...
		do
		{
			...
			if (effect >= 0)
			{
				for (var spellEffectInfo : spellInfo.getEffects())
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("The spell %1$s Effect: %2$s listed in `spell_linked_spell` has same bp%3$s like effect (possible hack)", Math.abs(trigger), Math.abs(effect), spellEffectInfo.effectIndex));
					}
				}
			}
			...
			{
				Log.outError(LogFilter.Sql, String.format("The spell trigger %1$s, effect %2$s listed in `spell_linked_spell` has invalid link type %3$s, skipped.", trigger, effect, type));
			...
			if (trigger < 0)
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("The spell trigger %1$s listed in `spell_linked_spell` has invalid link type %2$s, changed to 0.", trigger, type));
				}
			...
			if (type != SpellLinkedType.aura)
			{
				if (trigger == effect)
				{
					Log.outError(LogFilter.Sql, String.format("The spell trigger %1$s, effect %2$s listed in `spell_linked_spell` triggers itself (infinite loop), skipped.", trigger, effect));
...
//			}
	...
	public void loadSpellInfoServerside()
	{
		...
		MultiMap<(int spellId, Difficulty difficulty), SpellEffectRecord> spellEffects = new MultiMap<(int spellId, Difficulty difficulty), SpellEffectRecord>();
		...
		if (!effectsResult.isEmpty())
		{
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s effext index %3$s references a regular spell loaded from file. Adding serverside effects to existing spells is not allowed.", spellId, difficulty, effect.effectIndex));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s effect index %2$s references non-existing difficulty %3$s, skipped", spellId, effect.effectIndex, difficulty));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid effect type %3$s at index %4$s, skipped", spellId, difficulty, effect.effect, effect.effectIndex));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid aura type %3$s at index %4$s, skipped", spellId, difficulty, effect.EffectAura, effect.effectIndex));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid targetA type %3$s at index %4$s, skipped", spellId, difficulty, effect.ImplicitTarget[0], effect.effectIndex));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid targetB type %3$s at index %4$s, skipped", spellId, difficulty, effect.ImplicitTarget[1], effect.effectIndex));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid radius id %3$s at index %4$s, set to 0", spellId, difficulty, effect.EffectRadiusIndex[0], effect.effectIndex));
				}

				if (effect.EffectRadiusIndex[1] != 0 && !CliDB.SpellRadiusStorage.HasRecord(effect.EffectRadiusIndex[1]))
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s has invalid max radius id %3$s at index %4$s, set to 0", spellId, difficulty, effect.EffectRadiusIndex[1], effect.effectIndex));
				}
		...
		if (!spellsResult.isEmpty())
		{
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Serverside spell %1$s difficulty %2$s is already loaded from file. Overriding existing spells is not allowed.", spellId, difficulty));
	...
	public void loadSpellInfoCustomAttributes()
	{
		...
		else
		{
			...
			do
			{
				...
				{
					if (attributes.HasAnyFlag((int)SpellCustomAttributes.ShareDamage.getValue()))
					{
...
// C# TO JAVA CONVERTER TASK: There is no 'goto' in Java:
								...
								default:
								{
									...
									// Frostbolt
									if (spellInfo.spellFamilyName == SpellFamilyNames.Mage && spellInfo.SpellFamilyFlags[0].HasAnyFlag(0x20))
									{
										break;
									}
									...
									// Haunt
									if (spellInfo.spellFamilyName == SpellFamilyNames.Warlock && spellInfo.SpellFamilyFlags[1].HasAnyFlag(0x40000))
									{
										break;
									}
				...
				switch (spellInfo.spellFamilyName)
				{
					case SpellFamilyNames.Warrior:
						// Shout / Piercing Howl
						if (spellInfo.SpellFamilyFlags[0].HasAnyFlag(0x20000))
						{
							spellInfo.AttributesCu |= SpellCustomAttributes.AuraCC;
						}
					...
					case SpellFamilyNames.Druid:
						// Roar
						if (spellInfo.SpellFamilyFlags[0].HasAnyFlag(0x8))
						{
							spellInfo.AttributesCu |= SpellCustomAttributes.AuraCC;
						}
...
//						}
	...
	public void loadSpellTotemModel()
	{
		...
		do
		{
			...
			if (spellEntry == null)
			{
				Log.outError(LogFilter.Sql, String.format("SpellID: %1$s in `spell_totem_model` table could not be found in dbc, skipped.", spellId));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Race %1$s defined in `spell_totem_model` does not exists, skipped.", race));
			...
			{
				Log.outError(LogFilter.Sql, String.format("SpellID: %1$s defined in `spell_totem_model` has non-existing model (%2$s).", spellId, displayId));
