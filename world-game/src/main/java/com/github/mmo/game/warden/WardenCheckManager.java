...
public class WardenCheckManager 
{
	...
	public final void loadWardenChecks()
	{
		...
		do
		{
			...
			{
				Log.outError(LogFilter.Sql, String.format("Warden check with id %1$s lists check type %2$s in `warden_checks`, which is not supported. Skipped.", id, checkType));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Warden Lua check with id %1$s found in `warden_checks`. Lua checks may have four-digit IDs at most. Skipped.", id));
			...
			if (checkType == WardenCheckType.LuaEval)
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Found over-long Lua check for Warden check with id %1$s in `warden_checks`. Max length is %2$s. Skipped.", id, WARDEN_MAX_LUA_CHECK_LENGTH));
