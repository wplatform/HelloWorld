...
import com.github.mmo.game.entity.gobject.GameObjectTemplate;
	...
	public final void loadTransportSpawns()
	{
		...
		if (!result.isEmpty())
		{
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) with unknown gameobject `entry` set, skipped.", guid, entry));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) with unknown `phaseUseFlags` set, removed unknown value.", guid, entry));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) has both `phaseUseFlags` PHASE_USE_FLAGS_ALWAYS_VISIBLE and PHASE_USE_FLAGS_INVERSE, removing PHASE_USE_FLAGS_INVERSE.", guid, entry));
				...
				{
					Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) with both `phaseid` and `phasegroup` set, `phasegroup` set to 0", guid, entry));
					phaseGroupId = 0;
				}
				...
				if (phaseId != 0)
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) with `phaseid` %3$s does not exist, set to 0", guid, entry, phaseId));
						phaseId = 0;
					}
				}
				...
				if (phaseGroupId != 0)
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("Table `transports` have transport (GUID: %1$s Entry: %2$s) with `phaseGroup` %3$s does not exist, set to 0", guid, entry, phaseGroupId));
						phaseGroupId = 0;
					}
				}
...
//			}
	...
	_ = tempOut__.outArgValue;
		...
		{
			Log.outError(LogFilter.Sql, String.format("Transport %1$s will not be loaded, failed to compute starting position", entry));
