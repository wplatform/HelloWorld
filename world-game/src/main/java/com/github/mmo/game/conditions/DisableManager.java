		...
		switch (type)
		{
			case Spell:
			{
				...
				if (refe != null)
				{
					...
					{
						if (spellFlags.HasAnyFlag(DisableFlags.SPELLARENAS.getValue() | DisableFlags.SPELLBATTLEGROUNDS.getValue()))
						{
							var map = refe.getMap();
				...
				else if (flags.HasAnyFlag((byte)DisableFlags.SPELLLOS.getValue()))
				{
