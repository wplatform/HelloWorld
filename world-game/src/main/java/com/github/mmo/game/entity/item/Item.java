...
import com.github.mmo.game.entity.ArtifactPower;
...
public class Item extends WorldObject
{
	...
	private com.github.mmo.game.entity.item.BonusData bonusData;
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	public final boolean gemsFitSockets()
	{
		...
		for (var gemData : getItemData().gems)
		{
			...
			if (!GemColor.HasAnyFlag(ItemConst.SocketColorToGemTypeMask[SocketColor.getValue()])) // bad gem color on this socket
			{
				return false;
			}
		}
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'ref' keyword - these cannot be converted using the 'RefObject' helper class unless the method is within the code being modified:
	...
	public static void removeItemsSetItem(Player player, Item item)
	{
		...
		if (set == null)
		{
			Log.outError(LogFilter.Sql, String.format("Item set %1$s for item %2$s not found, mods not removed.", setid, item.getEntry()));
