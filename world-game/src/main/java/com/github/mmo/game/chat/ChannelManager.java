...
import com.github.mmo.game.entity.ObjectGuidGenerator;
...
{
	private static final channelManager allianceChannelMgr = new channelManager(TeamFaction.Alliance);
	private static final ChannelManager hordeChannelMgr = new channelManager(TeamFaction.Horde);
	...
	public static void loadFromDB()
	{
		...
		ArrayList<(String name, TeamFaction team)> toDelete = new ArrayList<(String name, TeamFaction team)>();
		int count = 0;

		do
		{
			var dbName = result.<String>Read(0); // may be different - channel names are case insensitive
			var team = TeamFaction.forValue(result.<Integer>Read(1));
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
	...
	private ObjectGuid createCustomChannelGuid()
	{
		...
		high |= (long)(team == TeamFaction.Alliance ? 3 : 5) << 4;
	...
	private ObjectGuid createBuiltinChannelGuid(int channelId, AreaTableRecord zoneEntry)
	{
		...
		if (channelEntry.flags.HasAnyFlag(ChannelDBCFlags.global.getValue() | ChannelDBCFlags.CityOnly.getValue()))
		{
			zoneId = 0;
		}
