...
public class Creature extends Unit implements GridObject<Creature>, MapObject {
    ...
    private reactStates reactState = ReactStates.values()[0];
    ...
    public Creature(boolean worldObject) {
        ...
        setSightDistance(SharedConst.SightRangeUnit);
...
// C# TO JAVA CONVERTER TASK: There is no Java ConcurrentHashMap equivalent to this .NET ConcurrentDictionary method:
    ...
    public final boolean updateEntry(int entry) {
        ...
        if (!isGuardian()) {
            ...
            setStatFlatModifier(UnitMods.ResistanceHoly, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Holy.getValue()]);
            setStatFlatModifier(UnitMods.ResistanceFire, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Fire.getValue()]);
            setStatFlatModifier(UnitMods.ResistanceNature, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Nature.getValue()]);
            setStatFlatModifier(UnitMods.ResistanceFrost, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Frost.getValue()]);
            setStatFlatModifier(UnitMods.ResistanceShadow, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Shadow.getValue()]);
            setStatFlatModifier(UnitMods.ResistanceArcane, UnitModifierFlatType.base, cInfo.Resistance[SpellSchools.Arcane.getValue()]);
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
    ...
    public final void selectLevel() {
        ...
        var level = (minlevel == maxlevel ? minlevel : RandomUtil.IRand(minlevel, maxlevel));
        setLevel(level);
    ...
    public final void updateLevelDependantStats() {
        ...
        var level = getLevel();
        var stats = global.getObjectMgr().getCreatureBaseStats(level, cInfo.unitClass);
        ...
        setStatFlatModifier(UnitMods.attackPower, UnitModifierFlatType.base, stats.getAttackPower());
        setStatFlatModifier(UnitMods.AttackPowerRanged, UnitModifierFlatType.base, stats.getRangedAttackPower());
...
//			}
    ...
    public final void applyLevelScaling() {
        var scaling = getTemplate().getLevelScaling(getMap().getDifficultyID());
        var levels = global.getDB2Mgr().GetContentTuningData(scaling.contentTuningId, 0);

        if (levels != null) {
            setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().scalingLevelMin), levels.getValue().minLevel);
            setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().scalingLevelMax), levels.getValue().maxLevel);
        } else if (ConfigMgr.GetDefaultValue("CreatureScaling.DefaultMaxLevel", false)) {
            setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().scalingLevelMin), 1);
            setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().scalingLevelMax), WorldConfig.getIntValue(WorldCfg.MaxPlayerLevel));
        }
        ...
        var delta = mindelta == maxdelta ? mindelta : RandomUtil.IRand(mindelta, maxdelta);

        setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().scalingLevelDelta), delta);
        setUpdateFieldValue(getValues().modifyValue(getUnitData()).modifyValue(getUnitData().contentTuningID), scaling.contentTuningId);
    }
    ...
    public final boolean hasReactState(ReactStates state) {
...
// C# TO JAVA CONVERTER TASK: Tuple fields are not converted by C# to Java Converter:
    ...
    private float getBaseArmorForLevel(int level) {
        var cInfo = getTemplate();
        var scaling = cInfo.getLevelScaling(getMap().getDifficultyID());
        var baseArmor = global.getDB2Mgr().EvaluateExpectedStat(ExpectedStatType.CreatureArmor, level, cInfo.getHealthScalingExpansion(), scaling.contentTuningId, playerClass.forValue(cInfo.unitClass));
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
    ...
    public final void setReactState(ReactStates value) {
        reactState = value;
    }
