...
import com.github.mmo.game.scripting.interfaces.ibattlefield.IBattlefieldGetBattlefield;
...
public class BattleFieldManager 
{
	...
	private final HashMap<(Map map, int zoneId), BattleField> battlefieldsByZone = new HashMap<(Map map, int zoneId), BattleField>();
	...
	public final void initBattlefield()
	{
		...
		if (!result.isEmpty())
		{
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("BattlefieldMgr::InitBattlefield: Invalid TypeId value %1$s in battlefield_template, skipped.", typeId));
