...
import com.github.mmo.defines.GOState;
...
import com.github.mmo.game.entity.creature.enums.MapObjectCellMoveState;
import com.github.mmo.game.entity.object.MapObject;
import com.github.mmo.game.map.collision.model.GameObjectModel;
import com.github.mmo.game.entity.gameObjectFieldData;
import com.github.mmo.game.entity.ObjectFieldData;
...
public class GameObject extends WorldObject implements GridObject<GameObject> , MapObject
{
	...
	private LootState lootState = getLootState().values()[0];
	private ObjectGuid lootStateUnitGuid = ObjectGuid.EMPTY; // GUID of the unit passed with setLootState(LootState, Unit*)
	...
	public final void activateObject(GameObjectActions action, int param, WorldObject spellCaster, int spellId, int effectIndex)
	{
		...
		switch (action)
		{
			...
			case UseArtKit4:
			{
				...
				if (artKitValue == 0)
				{
					Log.outError(LogFilter.Sql, String.format("GameObject %1$s hit by spell %2$s needs `artkit%3$s` in `gameobject_template_addon`", getEntry(), spellId, artKitIndex));
				}
				else
				{
			...
			case Chair: //7
			{
				...
				if (chairListSlots.isEmpty()) // this is called once at first chair use to make list of available slots
				{
					if (info.chair.chairslots > 0) // sometimes chairs in DB have error in fields and we dont know number of slots
					{
						...
						{
							chairListSlots.put(i, default); // Last user of current slot set to 0 (none sit here yet)
						}
					}
					else
					{
						chairListSlots.put(0, default); // error in DB, make one default slot
					}
				}
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
	...
	{
		var transport = _goTypeImpl instanceof com.github.mmo.game.entity.gameobjecttype.Transport ? (com.github.mmo.game.entity.gameobjecttype.transport)_goTypeImpl : null;
	...
	public final ITransport toTransportBase()
	{
		...
		{
			case Transport:
				return (com.github.mmo.game.entity.gameobjecttype.transport)goTypeImpl;
	...
	private boolean create(int entry, Map map, Position pos, Quaternion rotation, int animProgress, GOState goState, int artKit, boolean dynamic, long spawnid)
	{
		...
		if (goInfo == null)
		{
			Log.outError(LogFilter.Sql, "Gameobject (Spawn id: {0} Entry: {1}) not created: non-existing entry in `gameobject_template`. Map: {2} (X: {3} Y: {4} Z: {5})", getSpawnId(), entry, map.getId(), pos.getX(), pos.getY(), pos.getZ());
		...
		switch (goInfo.type)
		{
			...
			case DestructibleBuilding:
				...
				break;
			case Transport:
				goTypeImpl = new com.github.mmo.game.entity.gameobjecttype.transport(this);
