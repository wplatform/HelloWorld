...
import com.github.mmo.game.TerrainSwapInfo;
...
public class PhaseShift
{
	...
	public final boolean canSee(PhaseShift other)
	{
		...
		if (!flags.hasFlag(PhaseShiftFlag.INVERSE) && !other.flags.hasFlag(PhaseShiftFlag.INVERSE))
		{
			...
			return phases.Intersect(other.phases, (myPhase, otherPhase) ->
			{
											...
											return !myPhase.value.flags.HasAnyFlag(excludePhasesWithFlag) && (!myPhase.value.flags.hasFlag(phaseFlags.PERSONAL) || Objects.equals(ownerGuid, otherPersonalGuid));
			}).Any();
		}
	...
	{
		var unphasedFlag = !flags.HasAnyFlag(PhaseShiftFlag.INVERSE) ? PhaseShiftFlag.Unphased : PhaseShiftFlag.INVERSEUNPHASED;
