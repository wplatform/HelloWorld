...
public class SpellTargetSelector implements ICheck<unit>
{
	...
	public final boolean invoke(Unit target)
	{
		...
		{
			if (spellInfo.getRangeEntry().flags.HasAnyFlag(SpellRangeFlag.Melee))
			{
			...
			else
			{
				...
				if (spellInfo.getRangeEntry().flags.HasAnyFlag(SpellRangeFlag.Ranged))
				{
				...
				if (minRange > 0.0f && !spellInfo.getRangeEntry().flags.HasAnyFlag(SpellRangeFlag.Ranged))
				{
					minRange += rangeMod;
				}
			}

			if (caster.isMoving() && target.isMoving() && !caster.isWalking() && !target.isWalking() && (spellInfo.getRangeEntry().flags.HasAnyFlag(SpellRangeFlag.Melee) || target.isTypeId(TypeId.PLAYER)))
			{
				rangeMod += 8.0f / 3.0f;
			}
		}
