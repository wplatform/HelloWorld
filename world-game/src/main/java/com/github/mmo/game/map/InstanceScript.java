...
import com.github.mmo.game.InstanceSpawnGroupInfo;
import game.PhasingHandler;
import com.github.mmo.game.ai.BossAI;
...
public class InstanceScript extends ZoneScript
{
	...
	private void updateSpawnGroups()
	{
		...
		for (var info : instanceSpawnGroups)
		{
			...
			if (((getInstance().getTeamIdInInstance() == TeamIds.Alliance) && info.flags.hasFlag(InstanceSpawnGroupFlags.HordeOnly)) || ((getInstance().getTeamIdInInstance() == TeamIds.Horde) && info.flags.hasFlag(InstanceSpawnGroupFlags.AllianceOnly)))
			{
				continue;
			}
