package com.github.mmo.game.battlepet;
...
public class BattlePetMgr
{
	...
	private static void loadDefaultPetQualities()
	{
		...
		do
		{
			...
			if (battlePetSpecies == null)
			{
				Log.outError(LogFilter.Sql, String.format("Non-existing BattlePetSpecies.db2 entry %1$s was referenced in `battle_pet_quality` by row (%2$s, %3$s).", speciesId, speciesId, quality));
			...
			{
				Log.outError(LogFilter.Sql, String.format("BattlePetSpecies.db2 entry %1$s was referenced in `battle_pet_quality` with non-existing quality %2$s).", speciesId, quality));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Learnable BattlePetSpecies.db2 entry %1$s was referenced in `battle_pet_quality` with invalid quality %2$s. Maximum allowed quality is BattlePetBreedQuality::Rare.", speciesId, quality));
