...
public class ItemEnchantmentManager
{
	...
	public static void loadItemRandomBonusListTemplates()
	{
		...
		do
		{
			...
			{
				Log.outError(LogFilter.Sql, String.format("Bonus list %1$s used in `item_random_bonus_list_template` by id %2$s doesn't have exist in itemBonus.db2", bonusListId, id));
			...
			{
				Log.outError(LogFilter.Sql, String.format("Bonus list %1$s used in `item_random_bonus_list_template` by id %2$s has invalid chance %3$s", bonusListId, id, chance));
	...
	public static int generateItemRandomBonusListId(int item_id)
	{
		...
		if (tab == null)
		{
			Log.outError(LogFilter.Sql, String.format("Item RandomBonusListTemplateId id %1$s used in `item_template_addon` but it does not have records in `item_random_bonus_list_template` table.", itemProto.getRandomBonusListTemplateId()));
