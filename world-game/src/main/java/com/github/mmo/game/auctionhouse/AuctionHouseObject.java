...
import com.github.mmo.game.scripting.interfaces.iauctionhouse.*;
import java.util.*;
import java.time.*;
...
public class AuctionHouseObject
{
	...
	public final void addAuction(SQLTransaction trans, AuctionPosting auction)
	{
		...
		AuctionPosting.Sorter insertSorter = new AuctionPosting.Sorter(locale.enUS, new AuctionSortDef[] {new(AuctionHouseSortOrder.price, false)}, 1);
	...
	public final void buildListBuckets(AuctionListBucketsResult listBucketsResult, Player player, String name, byte minLevel, byte maxLevel, AuctionHouseFilterMask filters, AuctionSearchClassFilters classFilters, byte[] knownPetBits, int knownPetBitsCount, byte maxKnownPetLevel, int offset, AuctionSortDef[] sorts, int sortCount)
	{
		...
		for (var bucket : buckets.entrySet())
		{
			...
			if (classFilters != null)
			{
				...
				{
					if (!classFilters.Classes[bucketData.ItemClass].subclassMask.HasAnyFlag(AuctionSearchClassFilters.FilterType.forValue(1 << bucketData.itemSubClass)))
					{
						continue;
					}

					if (!classFilters.Classes[bucketData.ItemClass].InvTypes[bucketData.ItemSubClass].HasAnyFlag(1 << bucketData.inventoryType))
					{
						continue;
					}
				}
