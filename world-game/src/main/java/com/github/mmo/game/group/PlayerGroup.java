...
public class PlayerGroup
{
	...
	private GroupFlags groupFlags = getGroupFlags().values()[0];
	private GroupCategory groupCategory = getGroupCategory().values()[0];
	...
	private LootMethod lootMethod = getLootMethod().values()[0];
	private ItemQuality lootThreshold = itemQuality.values()[0];
    private ObjectGuid looterGuid = ObjectGuid.EMPTY;
    private ObjectGuid masterLooterGuid = ObjectGuid.EMPTY;
	...
	{
		return groupFlags.HasAnyFlag(GroupFlags.Lfg);
	}

	public final boolean isRaidGroup()
	{
		return groupFlags.HasAnyFlag(GroupFlags.raid);
	}
