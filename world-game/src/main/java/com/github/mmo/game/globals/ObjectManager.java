...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
    ...
    public void loadQuests() {
        ...
        try (var items = questRepository.streamAllQuestDetails()) {
            items.forEach(fields -> {
                ...
                for (int i = 0; i < QuestInfo.QUEST_EMOTE_COUNT; ++i) {
                    ...
                    questInfo.detailsEmote[i] = fields[1 + i];
                }

                System.arraycopy(fields, 5, questInfo.detailsEmoteDelay, 0, QuestInfo.QUEST_EMOTE_COUNT);
            });
        }
        ...
        try (var items = questRepository.streamAllQuestRequestItems()) {
            items.forEach(fields -> {
                QuestInfo questInfo = tmp.get((Integer) fields[0]);
                if (questInfo == null) {
                    Logs.SQL.error("Table `quest_request_items` has data for quest {} but such quest does not exist", fields[0]);
                    return;
                }
                questInfo.emoteOnComplete = (Integer) fields[1];
                questInfo.emoteOnIncomplete = (Integer) fields[2];

                if (dbcObjectManager.emote(questInfo.emoteOnComplete) == null)
                    Logs.SQL.error("Table `quest_request_items` has non-existing EmoteOnComplete ({}) set for quest {}.", questInfo.emoteOnComplete, fields[0]);

                if (dbcObjectManager.emote(questInfo.emoteOnIncomplete) == null)
                    Logs.SQL.error("Table `quest_request_items` has non-existing EmoteOnIncomplete ({}) set for quest {}.", questInfo.emoteOnIncomplete, fields[0]);

                questInfo.emoteOnCompleteDelay = (Integer) fields[3];
                questInfo.emoteOnIncompleteDelay = (Integer) fields[4];
                questInfo.requestItemsText.set(Locale.enUS, (String) fields[5]);
            });
        }
        ...
        try (var items = questRepository.streamAllQuestOfferReward()) {
            items.forEach(fields -> {
                ...
                for (int i = 0; i < QuestInfo.QUEST_EMOTE_COUNT; ++i) {
                    ...
                    questInfo.offerRewardEmote[i] = (Integer) fields[1 + i];
                }

                for (int i = 0; i < QuestInfo.QUEST_EMOTE_COUNT; ++i)
                    questInfo.offerRewardEmoteDelay[i] = (Integer) fields[5 + i];

                questInfo.offerRewardText.set(Locale.enUS, (String) fields[9]);
            });
        }
        ...
        try (var items = questRepository.streamAllQuestTemplateAddon()) {
            items.forEach(fields -> {
                ...
                questInfo.maxLevel = (Short) fields[1];
                questInfo.allowableClasses = (Integer) fields[2];
                questInfo.sourceSpellID = (Integer) fields[3];
                questInfo.prevQuestId = (Integer) fields[4];
                questInfo.nextQuestId = (Integer) fields[5];
                questInfo.exclusiveGroup = (Integer) fields[6];
                questInfo.breadcrumbForQuestId = (Integer) fields[7];
                questInfo.rewardMailTemplateId = (Integer) fields[8];
                questInfo.rewardMailDelay = (Integer) fields[9];
                questInfo.requiredSkillId = (Integer) fields[10];
                questInfo.requiredSkillPoints = (Integer) fields[11];
                questInfo.requiredMinRepFaction = (Integer) fields[12];
                questInfo.requiredMaxRepFaction = (Integer) fields[13];
                questInfo.requiredMinRepValue = (Integer) fields[14];
                questInfo.requiredMaxRepValue = (Integer) fields[15];
                questInfo.sourceItemIdCount = (Integer) fields[16];
                questInfo.specialFlags.setFlags((Integer) fields[17]);
                questInfo.scriptId = getScriptId((String) fields[18]);

                if (questInfo.specialFlags.hasFlag(QuestSpecialFlag.AUTO_ACCEPT))
                    questInfo.flags.addFlag(QuestFlag.AUTO_ACCEPT);
            });
        }
        ...
        try (var items = questRepository.streamAllQuestMailSender()) {
            items.forEach(fields -> {
                ...
                questInfo.rewardMailSenderEntry = fields[1];
            });
        }
        ...
        try (var items = questRepository.streamAllQuestVisualEffect()) {
            items.forEach(fields -> {
                QuestInfo questInfo = tmp.get(fields[2]);
                if (questInfo == null) {
                ...
                for (QuestObjective obj : questInfo.objectives) {
                    if (obj.id == objID) {
        ...
        for (var qinfo : tmp.values()) {
            ...
            for (var j = 0; j < QuestInfo.QUEST_ITEM_DROP_COUNT; j++) {
            ...
            for (var j = 0; j < QuestInfo.QUEST_REWARD_CHOICES_COUNT; ++j) {
                var id = qinfo.rewardChoiceItemId[j];
            ...
            for (var j = 0; j < QuestInfo.QUEST_REWARD_ITEM_COUNT; ++j) {
            ...
            for (var j = 0; j < QuestInfo.QUEST_REWARD_REPUTATIONS_COUNT; ++j) {
            ...
            for (var j = 0; j < QuestInfo.QUEST_REWARD_CURRENCY_COUNT; ++j) {
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
    ...
    public void initializeQueriesData(QueryDataGroup mask) {
        ...
        if (!WorldConfig.getBoolValue(WorldCfg.CacheDataQueries)) {
            Logs.SERVER_LOADING.info("Query data caching is disabled. Skipped initialization.");

            return;
        }

        // Initialize Query data for creatures
        if (mask.hasFlag(QueryDataGroup.Creatures)) {
        ...
        if (mask.hasFlag(QueryDataGroup.Gameobjects)) {
        ...
        if (mask.hasFlag(QueryDataGroup.Quests)) {
        ...
        if (mask.hasFlag(QueryDataGroup.POIs)) {
            for (var poiPair : questPOIStorage.entrySet()) {
                poiPair.getValue().initializeQueryData();
            }
        }

        Logs.SERVER_LOADING.info(String.format("Initialized query cache data in %1$s ms", time.GetMSTimeDiffToNow(oldMSTime)));
    }
...
// C# TO JAVA CONVERTER TASK: The following method call contained an unresolved 'out' keyword - these cannot be converted using the 'OutObject' helper class unless the method is within the code being modified:
    ...
    private void loadTerrainWorldMaps() {
        ...
        if (result.isEmpty()) {
            Logs.SERVER_LOADING.info("Loaded 0 terrain world maps. DB table `terrain_worldmap` is empty.");
        ...
        do {
            ...
            if (!CliDB.MapStorage.containsKey(mapId)) {
                Logs.SQL.error("TerrainSwapMap {0} defined in `terrain_worldmap` does not exist, skipped.", mapId);

                continue;
            }

            if (!global.getDB2Mgr().IsUiMapPhase((int) uiMapPhaseId)) {
                Logs.SQL.error(String.format("Phase %1$s defined in `terrain_worldmap` is not a valid terrain swap phase, skipped.", uiMapPhaseId));
        ...
        Logs.SERVER_LOADING.info("Loaded {0} terrain world maps in {1} ms.", count, time.GetMSTimeDiffToNow(oldMSTime));
    }
    ...
    private void loadTerrainSwapDefaults() {
        ...
        if (result.isEmpty()) {
            Logs.SERVER_LOADING.info("Loaded 0 terrain swap defaults. DB table `terrain_swap_defaults` is empty.");
        ...
        do {
            ...
            if (!CliDB.MapStorage.containsKey(mapId)) {
                Logs.SQL.error("Map {0} defined in `terrain_swap_defaults` does not exist, skipped.", mapId);
            ...
            if (!CliDB.MapStorage.containsKey(terrainSwap)) {
                Logs.SQL.error("TerrainSwapMap {0} defined in `terrain_swap_defaults` does not exist, skipped.", terrainSwap);
        ...
        Logs.SERVER_LOADING.info("Loaded {0} terrain swap defaults in {1} ms.", count, time.GetMSTimeDiffToNow(oldMSTime));
    }
    ...
    private void loadAreaPhases() {
        ...
        if (result.isEmpty()) {
            Logs.SERVER_LOADING.info("Loaded 0 phase areas. DB table `phase_area` is empty.");
...
//			}
        ...
        do {
            ...
            if (!CliDB.AreaTableStorage.containsKey(area)) {
                Logs.SQL.error(String.format("Area %1$s defined in `phase_area` does not exist, skipped.", area));

                continue;
            }

            if (!CliDB.PhaseStorage.containsKey(phaseId)) {
                Logs.SQL.error(String.format("Phase %1$s defined in `phase_area` does not exist, skipped.", phaseId));
        ...
        Logs.SERVER_LOADING.info(String.format("Loaded %1$s phase areas in %2$s ms.", count, time.GetMSTimeDiffToNow(oldMSTime)));
    }
