...
import com.github.mmo.game.GameEventData;
import com.github.mmo.game.GameEventQuestToEventConditionNum;
import com.github.mmo.game.GameEventState;
import com.github.mmo.game.ModelEquip;
...
public class GameEventManager 
{
	...
	private int[] mGameEventBattlegroundHolidays;
	private ArrayList<(long guid, long npcflag)>[] mGameEventNPCFlags;
	...
	{
		{
			...
			do
			{
				...
				{
					Log.outError(LogFilter.Sql, String.format("`game_event` game event id (%1$s) isn't a world event and has length = 0, thus it can't be used.", event_id));
				...
				if (pGameEvent.holiday_id != HolidayIds.NONE)
				{
					...
					{
						Log.outError(LogFilter.Sql, String.format("`game_event` game event id (%1$s) contains nonexisting holiday id %2$s.", event_id, pGameEvent.holiday_id));
...

		{
			...
			else
			{
				...
				do
				{
					...
					if (data.poolId != 0)
					{
						Log.outError(LogFilter.Sql, String.format("`game_event_creature`: game event id (%1$s) contains creature (%2$s) which is part of a pool (%3$s). This should be spawned in game_event_pool", event_id, guid, data.poolId));
					}
...

		{
			...
			else
			{
				...
				do
				{
					...
					if (data.poolId != 0)
					{
						Log.outError(LogFilter.Sql, String.format("`game_event_gameobject`: game event id (%1$s) contains game object (%2$s) which is part of a pool (%3$s). This should be spawned in game_event_pool", event_id, guid, data.poolId));
					}
			...
			else
			{
				...
				do
				{
					...
					if (newModelEquipSet.equipment_id > 0)
					{
						...
						{
							Log.outError(LogFilter.Sql, "Table `game_event_model_equip` have creature (Guid: {0}, entry: {1}) with equipment_id {2} not found in table `creature_equip_template`, set to no equipment.", guid, entry, newModelEquipSet.equipment_id);
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
	...
	private void setHolidayEventTime(GameEventData gameEvent)
	{
		...
		{
			Log.outError(LogFilter.Sql, String.format("Missing date or duration for holiday %1$s.", gameEvent.holiday_id));
