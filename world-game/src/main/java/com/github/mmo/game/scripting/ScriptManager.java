...
import com.github.mmo.game.scripting.activators.*;
...
import com.github.mmo.game.scripting.registers.*;
...
// C# TO JAVA CONVERTER TASK: There is no preprocessor in Java:
	...
	private void loadScriptWaypoints()
	{
		...
		do
		{
			...
			if (info == null)
			{
				Log.outError(LogFilter.Sql, String.format("SystemMgr: DB table script_waypoint has waypoint for non-existant creature entry %1$s", entry));
			...
			if (info.scriptID == 0)
			{
				Log.outError(LogFilter.Sql, String.format("SystemMgr: DB table script_waypoint has waypoint for creature entry %1$s, but creature does not have ScriptName defined and then useless.", entry));
			}
