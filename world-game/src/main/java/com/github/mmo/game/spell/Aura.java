...
import com.github.mmo.game.networking.packet.SpellCastVisual;
...
//				}
	...
	public final void prepareProcChargeDrop(SpellProcEntry procEntry, ProcEventInfo eventInfo)
	{
		...
		if (!procEntry.getAttributesMask().HasAnyFlag(ProcAttributes.UseStacksForCharges) && isUsingCharges() && (eventInfo.getSpellInfo() == null || !eventInfo.getSpellInfo().hasAttribute(SpellAttr6.DoNotConsumeResources)))
		{
			--_procCharges;
			setNeedClientUpdateForTargets();
		}
	...
	public final HashSet<Integer> getProcEffectMask(AuraApplication aurApp, ProcEventInfo eventInfo, LocalDateTime now)
	{
		...
		if (isUsingCharges())
		{
			...
			if (procEntry.getAttributesMask().HasAnyFlag(ProcAttributes.ReqSpellmod))
			{
...
//		}
	...
	public final void handleAuraSpecificMods(AuraApplication aurApp, Unit caster, boolean apply, boolean onReapply)
	{
		...
		if (apply)
		{
			switch (getSpellInfo().getSpellFamilyName())
			{
				...
				case Druid:
					...
					// Rejuvenation
					if (getSpellInfo().getSpellFamilyFlags().get(0).HasAnyFlag(0x10) && getEffect(0) != null)
					{
		...
		else
		{
			switch (getSpellInfo().getSpellFamilyName())
			{
				...
				case Priest:
					...
					// Power word: shield
					if (removeMode == AuraRemoveMode.EnemySpell && getSpellInfo().getSpellFamilyFlags().get(0).HasAnyFlag(0x00000001))
					{
						// Rapture
		...
		switch (getSpellInfo().getSpellFamilyName())
		{
			...
			case Warlock:
				...
				if (getSpellInfo().getSpellFamilyFlags().get(0).HasAnyFlag(0x00004000))
				{
					if (caster == null)
					{
						break;
	...
	private double calcProcChance(SpellProcEntry procEntry, ProcEventInfo eventInfo)
	{
		...
		if (procEntry.getAttributesMask().HasAnyFlag(ProcAttributes.ReduceProc60) && eventInfo.getActor().getLevel() > 60)
		{
