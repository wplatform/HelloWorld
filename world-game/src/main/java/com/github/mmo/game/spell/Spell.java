...
import com.github.mmo.game.entity.dynamic.DynamicObjectType;
...
import com.github.mmo.game.entity.item.bonusData;
import com.github.mmo.game.entity.object.ObjectGuid;
import com.github.mmo.game.entity.object.WorldLocation;
import com.github.mmo.game.entity.object.WorldObject;
import com.github.mmo.game.entity.player.model.DuelInfo;
import com.github.mmo.game.entity.player.Player;
import com.github.mmo.game.entity.unit.*;
import com.github.mmo.game.entity.unit.enums.CurrentSpellType;
...
import com.github.mmo.game.networking.packet.spell.SpellCastVisual;
import com.github.mmo.game.scripting.SpellScript;
import com.github.mmo.game.scripting.interfaces.IHasSpellEffects;
import com.github.mmo.game.scripting.interfaces.ISpellScript;
import com.github.mmo.game.scripting.interfaces.iplayer.IPlayerOnDuelRequest;
import com.github.mmo.game.scripting.interfaces.iplayer.IPlayerOnQuestStatusChange;
import com.github.mmo.game.scripting.interfaces.iplayer.IPlayerOnSpellCast;
...
import com.github.mmo.game.spell.enums.SpellCastFlagsEx;
import com.github.mmo.game.spell.enums.SpellState;
...
public class Spell
{
	...
	{
		return triggeredCastFlags.HasAnyFlag(TriggerCastFlag.FullMask);
	}

	public final boolean isIgnoringCooldowns()
	{
		return triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnoreSpellAndCategoryCD);
	}
	...
	{
		return triggeredCastFlags.HasAnyFlag(TriggerCastFlag.DisallowProcEvents);
	}
...
public Spell(WorldObject caster, SpellInfo info, TriggerCastFlag triggerFlags, ObjectGuid originalCasterGuid, ObjectGuid originalCastId, Byte empoweredStage)
	{
		...
		{
			empowerStages.put(stage.getKey(), new() {Id = stage.getValue().id, durationMs = stage.getValue().durationMs, SpellEmpowerID = stage.getValue().SpellEmpowerID, stage = stage.getValue().Stage});
		}
	...
	public final void initExplicitTargets(SpellCastTargets targets)
	{
		...
		if (target != null)
		{
			...
			if ((target.toUnit() && !neededTargets.HasAnyFlag(SpellCastTargetFlags.UnitMask.getValue() | SpellCastTargetFlags.CorpseMask.getValue())) || (target.isTypeId(TypeId.gameObject) && !neededTargets.hasFlag(SpellCastTargetFlags.GameobjectMask)) || (target.isTypeId(TypeId.Corpse) && !neededTargets.hasFlag(SpellCastTargetFlags.CorpseMask)))
			{
		...
		else
		{
			...
			if (neededTargets.HasAnyFlag(SpellCastTargetFlags.UnitMask))
			{
				...
				else if (caster.isTypeId(TypeId.UNIT) && neededTargets.HasAnyFlag(SpellCastTargetFlags.UnitEnemy.getValue() | SpellCastTargetFlags.unit.getValue()))
				{
					unit = caster.toUnit().getVictim();
				}

				// didn't find anything - let's use self as target
				if (unit == null && neededTargets.HasAnyFlag(SpellCastTargetFlags.UnitRaid.getValue() | SpellCastTargetFlags.UnitParty.getValue().getValue() | SpellCastTargetFlags.UnitAlly.getValue().getValue()))
				{
					unit = caster.toUnit();
				}
	...
	{
		return uniqueTargetInfo.size()(targetInfo -> targetInfo.missCondition == SpellMissInfo.NONE && targetInfo.effects.contains(effect));
	}
	...
	{
		var aura_effmask = aura.buildEffectMaskForOwner(spellInfo, new HashSet<Integer>() {spellEffectInfo.EffectIndex}, unit);
	...
	param1 = tempRef_param1.refArgValue;
		...
		if (triggeredCastFlags.HasAnyFlag(TriggerCastFlag.CastDirectly) && (!spellInfo.isChanneled() || spellInfo.getMaxDuration() == 0))
		{
			cast(true);
		}
		else
		{
			...
			if (unitCaster != null)
			{
				...
				if (!triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnoreAuraInterruptFlags) && !spellInfo.hasAttribute(SpellAttr2.NotAnAction))
				{
			...
			if (!triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnoreGCD))
			{
	...
	public final void endEmpoweredSpell()
	{
		...
		if (getPlayerIfIsEmpowered(tempOut_p) && (spellInfo.getEmpowerStages().containsKey(empoweredSpellStage) && (var stageinfo = spellInfo.getEmpowerStages().get(empoweredSpellStage)) == var stageinfo)) // ensure stage is valid
		{
		p = tempOut_p.outArgValue;
			...
			while (spellInfo.getEmpowerStages().containsKey(nextStage) && (var nextStageinfo = spellInfo.getEmpowerStages().get(nextStage)) == var nextStageinfo)
			{
	...
	public final void sendCastResult(SpellCastResult result, Integer param1, Integer param2)
	{
		...
		if (triggeredCastFlags.HasAnyFlag(TriggerCastFlag.DontReportCastError))
		{
	...
	public final SpellCastResult checkCast(boolean strict, tangible.RefObject<Integer> param1, tangible.RefObject<Integer> param2)
	{
		...
		if (!spellInfo.isPassive())
		{
			...
			if (playerCaster != null)
			{
				...
				if (!triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnoreCasterAurastate))
				{
		...
		for (var spellEffectInfo : spellInfo.getEffects())
		{
			...
			switch (spellEffectInfo.effect)
			{
				...
				case Charge:
				{
					...
					if (!triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnoreCasterAuras) && unitCaster.hasUnitState(UnitState.Root))
					{
					...
					if (spellInfo.getNeedsExplicitUnitTarget())
					{
						...
						if (preGeneratedPath.getPathType().HasAnyFlag(PathType.SHORT))
						{
							return SpellCastResult.NOPATH;
						}
						else if (!result || preGeneratedPath.getPathType().HasAnyFlag(PathType.NOPATH.getValue() | PathType.INCOMPLETE.getValue()))
						{
							return SpellCastResult.NOPATH;
						}
...
//		}
	...
	private void selectExplicitTargets()
	{
		...
		if (target != null)
		{
			...
			if (spellInfo.getExplicitTargetMask().HasAnyFlag(SpellCastTargetFlags.UnitEnemy) || (spellInfo.getExplicitTargetMask().HasAnyFlag(SpellCastTargetFlags.unit) && !caster.isFriendlyTo(target)))
			{
				Unit redirect = null;
	...
	{
		selectImplicitChainTargets(spellEffectInfo, targetType, target, new HashSet<Integer>() {effIndex});
	}
	...
	private void selectImplicitTrajTargets(SpellEffectInfo spellEffectInfo, SpellImplicitTargetInfo targetType)
	{
		...
		for (var obj : targets)
		{
			...
			if (unitTarget)
			{
				...
				if (creatureTarget)
				{
					if (!creatureTarget.getTemplate().typeFlags.HasAnyFlag(CreatureTypeFlags.CollideWithMissiles))
					{
						continue;
					}
				}
	...
	private void searchTargets(IGridNotifier notifier, GridMapTypeMask containerMask, WorldObject referer, Position pos, float radius)
	{
		...
		var searchInWorld = containerMask.HasAnyFlag(GridMapTypeMask.CREATURE.getValue() | GridMapTypeMask.player.getValue().getValue() | GridMapTypeMask.Corpse.getValue().getValue().getValue() | GridMapTypeMask.gameObject.getValue().getValue().getValue());
	...
	{
		addUnitTarget(target, new HashSet<Integer>() {effIndex}, checkIfValid, implicit, losPosition);
	}
	...
	{
		addGOTarget(go, new HashSet<Integer>() {effIndex});
	}
	...
	{
		addItemTarget(item, new HashSet<Integer>() {effectIndex});
	}
	...
	{
		addCorpseTarget(corpse, new HashSet<Integer>() {effIndex});
	}
...
// C# TO JAVA CONVERTER TASK: There is no Java equivalent to 'unchecked' in this context:
	...
	private void sendSpellGo()
	{
		...
		if (caster.isTypeId(TypeId.PLAYER) && caster.toPlayer().getClass() == playerClass.Deathknight && hasPowerTypeCost(powerType.runes) && !triggeredCastFlags.HasAnyFlag(TriggerCastFlag.IgnorePowerAndReagentCost))
		{
...
//			}
		...
		if (unitflag.HasAnyFlag(UnitFlag.Stunned))
		{
		...
		else if (unitCaster.isSilenced(spellSchoolMask) && spellInfo.getPreventionType().HasAnyFlag(SpellPreventionType.Silence) && !checkSpellCancelsSilence(param1))
		{
			result = SpellCastResult.Silenced;
		}
		else if (unitflag.HasAnyFlag(UnitFlag.Pacified) && spellInfo.getPreventionType().HasAnyFlag(SpellPreventionType.Pacify) && !checkSpellCancelsPacify(param1))
		{
			result = SpellCastResult.Pacified;
		}
		else if (unitflag.HasAnyFlag(UnitFlag.Fleeing))
		{
		...
		else if (unitflag.HasAnyFlag(UnitFlag.Confused))
		{
		...
		else if (unitCaster.hasUnitFlag2(UnitFlag2.NoActions) && spellInfo.getPreventionType().HasAnyFlag(SpellPreventionType.NoActions) && !checkSpellCancelsNoActions(param1))
		{
...
// C# TO JAVA CONVERTER TASK: Java has no equivalent to C# deconstruction declarations:
		...
		if (target && target != caster)
		{
			...
			if (caster.isTypeId(TypeId.PLAYER) && ((spellInfo.getFacingCasterFlags().HasAnyFlag(1) && !caster.getLocation().hasInArc((float)Math.PI, target.getLocation())) && !caster.toPlayer().isWithinBoundaryRadius(target)))
			{
							...
							if (msg != InventoryResult.Ok)
							{
								...
								*/
								...
								else
								{
									...
									if (spellInfo.getSpellFamilyName() != SpellFamilyNames.Mage || (!spellInfo.getSpellFamilyFlags().get(0).HasAnyFlag(0x40000000)))
									{
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void effectApplyAura()
	{
		...
		if (aurApp == null)
		{
			aurApp = unitTarget._CreateAuraApplication(spellAura, new HashSet<Integer>() {EffectInfo.EffectIndex});
		}
		else
		{
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void effectSendEvent()
	{
		...
		else // if (effectHandleMode == SpellEffectHandleMode.hit)
		{
			...
			if (effectInfo.getProvidedTargetMask().HasAnyFlag(SpellCastTargetFlags.UnitMask.getValue() | SpellCastTargetFlags.GameobjectMask.getValue()))
			{
				return;
			}
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void effectHeal()
	{
		...
		*/
		...
		else if (spellInfo.getSpellFamilyName() == SpellFamilyNames.Deathknight && spellInfo.getSpellFamilyFlags().get(0).HasAnyFlag(0x00080000))
		{
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void effectSummonType()
	{
		...
		if (!properties.getFlags().HasAnyFlag(SummonPropertiesFlags.OnlyVisibleToSummoner.getValue() | SummonPropertiesFlags.OnlyVisibleToSummonerGroup.getValue()))
		{
	...
	private void effectSummonObjectWild()
	{
		...
		if (go.getGoType() == GameObjectTypes.flagDrop)
		{
			...
			if (player != null)
			{
				...
				if (bg)
				{
					bg.setDroppedFlagGUID(go.getGUID(), bg.getPlayerTeam(player.getGUID()) == TeamFaction.Alliance ? TeamIds.Horde : TeamIds.Alliance);
				}
			}
		}
...
// C# TO JAVA CONVERTER TASK: Java annotations will not correspond to .NET attributes:
	private void effectTransmitted()
	{
		...
		if (goinfo == null)
		{
			Log.outError(LogFilter.Sql, "Gameobject (Entry: {0}) not exist and not created at spell (ID: {1}) cast", name_id, spellInfo.getId());
