...
public class LootItem
{
	...
	public LootItem(LootStoreItem li)
	{
		...
		follow_loot_rules = !li.needs_quest || (proto != null && proto.getFlagsCu().HasAnyFlag(ItemFlagsCustom.FollowLootRules));
	...
	*/
	...
	public static boolean allowedForPlayer(Player player, Loot loot, int itemid, boolean needs_quest, boolean follow_loot_rules, boolean strictUsabilityCheck, ArrayList<Condition> conditions)
	{
		...
		if (!pProto.getFlagsCu().HasAnyFlag(ItemFlagsCustom.IgnoreQuestStatus) && ((needs_quest || (pProto.getStartQuest() != 0 && player.getQuestStatus(pProto.getStartQuest()) != QuestStatus.NONE)) && !player.hasQuestForItem(itemid)))
		{
			return false;
		}
